// routes/Access/changeHistoryRouter.js
//
// One change-history API, mounted once per department.
//
// WHY A FACTORY RATHER THAN A FILE PER DEPARTMENT
// ----------------------------------------------
// HR had this router first, written for HR. The accountant module needed the
// same six endpoints over the same collection, and copying the file would have
// produced two histories that drift — one gains a filter, the other does not,
// and the two departments quietly stop answering the same question the same way.
//
// It also matters for where this is heading. The plan is to eventually merge
// every department's history into one admin view; that is only cheap while the
// departments share a spine. `change_logs` already carries `departmentSlug`, so
// a merged view is this router with the department filter removed — not a
// rewrite.
//
// WHAT IS PARAMETERISED, AND WHAT IS NOT
// --------------------------------------
// The department, the auth middleware, and the mount path (for the docs in the
// banners) are arguments. The QUERY is not: every department reads the log the
// same way, and a per-department query is how two histories start disagreeing.
//
// SCOPED TO ONE DEPARTMENT, ALWAYS. `departmentSlug` is pinned from the factory
// argument and never read from the request. A department parameter would make
// this a way for an accounts viewer to read HR's salary changes by editing a
// query string, which is the sort of hole that only turns up in an audit of the
// audit log.
...
/**
 * @param {object} opts
 * @param {string} opts.department  slug pinned on every query ("hr", "accounting")
 * @param {function|function[]} opts.auth  middleware that must populate req.user
 * @param {string} [opts.mount]     the mount path, used only in comments/errors
 */
function createChangeHistoryRouter({ department, auth, mount = "" }) {
  const router = express.Router();
  const DEPARTMENT = String(department).toLowerCase();

  for (const mw of [].concat(auth || [])) router.use(mw);
   ...
   */
  const SECTION_KEYS = new Set(sectionsFor(DEPARTMENT).map((s) => s.key));

  function cleanSections(raw) {
    if (!raw) return [];
    const list = Array.isArray(raw) ? raw : String(raw).split(",");
    return list.map((x) => String(x).trim()).filter((x) => SECTION_KEYS.has(x));
  }
  ...
  router.get("/sections", async (req, res) => {
    try {
      ...
      console.error(`[${DEPARTMENT}/change-history] sections failed:`, err.message);
  ...
  router.get("/", async (req, res) => {
    try {
      ...
      console.error(`[${DEPARTMENT}/change-history] list failed:`, err.message);
  ...
  router.get("/actors", async (req, res) => {
    try {
      ...
      console.error(`[${DEPARTMENT}/change-history] actors failed:`, err.message);
  ...
  router.get("/stamps", async (req, res) => {
    try {
      ...
      console.error(`[${DEPARTMENT}/change-history] stamps failed:`, err.message);
      res.status(500).json({ success: false, message: "Could not load the record stamps." });
    }
  });

  /* ------------------------------------------------------------------ */
  /* GET <mount>/record/:entity/:entityId                 */
  /* ------------------------------------------------------------------ */
  ...
  router.get("/record/:entity/:entityId", async (req, res) => {
    try {
      ...
      console.error(`[${DEPARTMENT}/change-history] record failed:`, err.message);
  ...
  router.get("/summary", async (req, res) => {
    try {
      ...
      console.error(`[${DEPARTMENT}/change-history] summary failed:`, err.message);
  ...
  router.get("/export", async (req, res) => {
    try {
      ...
      console.error(`[${DEPARTMENT}/change-history] export failed:`, err.message);
