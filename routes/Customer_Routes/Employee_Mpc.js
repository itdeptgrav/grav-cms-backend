...
router.get("/", verifyCustomerToken, async (req, res) => {
  try {
    ...
    const departments = [
      ...new Set(deptAgg.map((d) => d._id.department).filter(Boolean)),
    ];
    const designations = [
      ...new Set(deptAgg.map((d) => d._id.designation).filter(Boolean)),
    ];
...
router.get("/export", verifyCustomerToken, async (req, res) => {
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      'attachment; filename="employees_export.csv"',
    );
    res.send(csv);
  } catch (error) {
  ...
  try {
    ...
    const toThumb = (u) =>
      u?.includes("/image/upload/")
        ? u.replace(
            "/image/upload/",
            "/image/upload/w_80,h_80,c_fill,q_70,f_webp/",
          )
        : u;

    const getImageExtension = (url) => {
      const ext = (
        url.match(/\.(png|jpg|jpeg|gif|webp)/i)?.[1] || "png"
      ).toLowerCase();
    ...
    for (const emp of employees) {
      ...
      row.height = hasAnyImage
        ? ROW_HEIGHT
        : Math.max(18, empProducts.length * 14);
...
async function getAssignedStockItemIds(customerId) {
  const customer = await Customer.findById(customerId)
    ...
    .lean();
  if (!customer) return null; // null = customer not found
...
// Strictly scoped to Customer.assignedStockItems.
router.get("/products/available", verifyCustomerToken, async (req, res) => {
  try {
    ...
    const products = await StockItem.find(filter)
      .select(
        "name reference category genderCategory baseSalesPrice totalQuantityOnHand images variants additionalNames",
      )
      .sort({ name: 1 })
      .limit(500) // hard cap; whitelist is already the practical limit
      .lean();

    // ── Attach matchedName (unchanged logic) ────────────────────────
    ...
    const enriched = products.map((p) => {
      ...
      if (search) {
        ...
        if (!canonicalMatches && Array.isArray(p.additionalNames)) {
          const hit = p.additionalNames.find((an) =>
            an.toLowerCase().includes(searchLower),
          );
...
// STRICTLY scoped to the customer's assignedStockItems. Products that
// were previously assigned to an employee but are no longer in the
// whitelist will not be returned here — they simply won't appear in the
// popup. Those orphans remain on the employee record (and can be removed
// using the row-level × button in the table).
router.post("/products/available", verifyCustomerToken, async (req, res) => {
  try {
    ...
    const uniqueIds = [...new Set(productIds.map(String))].slice(0, 500);
    ...
    // STRICT: filter requested IDs by the whitelist. Orphans are dropped.
    const assignedIdSet = new Set(assignedIds.map(String));
    const allowedIds = uniqueIds.filter((id) => assignedIdSet.has(id));

    if (!allowedIds.length) {
      return res.status(200).json({ success: true, products: [] });
    }

    const items = await StockItem.find({ _id: { $in: allowedIds } })
      .select(
        "name reference category genderCategory baseSalesPrice totalQuantityOnHand images variants additionalNames",
      )
      .lean();
...
router.patch("/bulk-products", verifyCustomerToken, async (req, res) => {
  try {
    ...
    for (const [idx, empData] of employees.entries()) {
      ...
      if (!_id) {
        results.errors.push(`Row ${idx + 1}: Missing employee ID`);
        continue;
      }

      const { validProducts, errors } = await buildValidProducts(
        products,
        `Employee ${idx + 1}: `,
      );
      if (errors.length > 0) {
...
router.post("/", verifyCustomerToken, async (req, res) => {
  try {
    ...
    if (!uin?.trim())
      return res
        .status(400)
        .json({ success: false, message: "UIN is required" });
    if (!gender)
      return res
        .status(400)
...
router.post("/batch", verifyCustomerToken, async (req, res) => {
  try {
    ...
    const allProductIds = [
      ...new Set(
        employees.flatMap((emp) =>
          (emp.products || []).map((p) => p.productId).filter(Boolean),
        ),
      ),
    ];
    ...
    if (employeesToCreate.length > 0) {
      try {
        createdEmployees = await EmployeeMpc.insertMany(employeesToCreate, {
          ordered: false,
        });
      } catch (error) {
        if (error.writeErrors?.length > 0) {
          error.writeErrors.forEach((writeError) => {
            if (writeError.code === 11000) {
              ...
              const failedRow =
                employees.findIndex((e) => e.uin?.toUpperCase() === failedUin) +
                1;
              skippedEmployees.push({
                row: failedRow,
                uin: failedUin,
...
router.put("/:id", verifyCustomerToken, async (req, res) => {
  try {
    const { name, uin, gender, department, designation, products, status } =
      req.body;
...
router.patch("/:id/status", verifyCustomerToken, async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Status updated", employee });
  } catch (error) {
