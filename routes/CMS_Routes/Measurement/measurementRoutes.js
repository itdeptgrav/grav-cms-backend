...
router.get("/organizations", async (req, res) => {
  try {
    ...
    const employeeMap = new Map(
      employeeStats.map((s) => [s._id.toString(), s]),
    );
    const measurementMap = new Map(
      measurementStats.map((s) => [s._id.toString(), s]),
    );
    ...
    res.status(200).json({
      ...
      stats: {
        ...
        organizationsWithEmployees: organizations.filter(
          (o) => o.employeeStats.totalEmployees > 0,
        ).length,
        organizationsWithMeasurements: organizations.filter(
          (o) => o.measurementStats.totalMeasurements > 0,
        ).length,
      },
    });
  } catch (error) {
    ...
    res.status(200).json({
      ...
      organization: {
        ...
        productStats: {
          ...
          uniqueProducts: productStats[0]?.uniqueProducts?.length || 0,
        },
...
async function ensureVariantAssigned(productData, stockItemDoc) {
  ...
  if (
    needsAutoAssign &&
    stockItemDoc.variants &&
    stockItemDoc.variants.length > 0
  ) {
    ...
    console.log(
      `✅ Auto-assigned variant for ${stockItemDoc.name}: ${variantName}`,
    );
  }
  ...
  if (measurementData?.length) {
    for (const data of measurementData) {
      ...
      if (data.variantId && stockItem.variants) {
        const v = stockItem.variants.find(
...
function computeIsCompleted(emp) {
  if (emp.noProductAssigned) {
    return (
      emp.categoryMeasurements?.length > 0 &&
...
router.post("/", async (req, res) => {
  try {
    ...
    if (!name?.trim())
      return res
        .status(400)
        .json({ success: false, message: "Measurement name is required" });
    ...
    if (measurementData?.length) {
      ...
      const stockItemMap = new Map(
        stockItems.map((s) => [s._id.toString(), s]),
      );

      processedMeasurementData = await Promise.all(
        measurementData.map(async (data) => {
          const stockItem = stockItemMap.get(data.productId?.toString());
          if (stockItem) {
    ...
    const employeeMeasurementsMap = await buildEmployeeMeasurementsMap(
      processedMeasurementData,
      categoryData,
    );

    const employeeMeasurements = Array.from(
      employeeMeasurementsMap.values(),
    ).map((emp) => {
    ...
    const measuredEmployees = employeeMeasurements.filter(
      (e) => e.isCompleted,
    ).length;
...
router.put("/:measurementId", async (req, res) => {
  try {
    ...
    const existingMap = new Map(
      measurement.employeeMeasurements.map((emp, idx) => [
    ...
    if (processedMeasurementData?.length) {
      ...
      const productIds = [
        ...new Set(
          processedMeasurementData.map((d) => d.productId).filter(Boolean),
        ),
      ];
      const stockItems = productIds.length
        ? await StockItem.find({ _id: { $in: productIds } })
            .select("_id name measurements variants")
            .lean()
        : [];
...
router.put("/:measurementId/add-employees", async (req, res) => {
  try {
    ...
    if (updatedRegisteredEmployeeIds?.length) {
      measurement.registeredEmployeeIds = [
        ...new Set(updatedRegisteredEmployeeIds),
      ];
    }

    const existingIds = new Set(
      measurement.employeeMeasurements.map((e) => e.employeeId.toString()),
    );
    ...
    if (categoryData?.length) {
      categoryData.forEach((catEmp) => {
        if (
          catEmp.employeeId &&
          !existingIds.has(catEmp.employeeId.toString())
        ) {
...
router.get("/organization/:orgId/measurements", async (req, res) => {
  try {
    ...
    const measurements = await Measurement.find({
      ...
      .select(
        "name description totalRegisteredEmployees measuredEmployees pendingEmployees completionRate createdAt updatedAt convertedToPO poRequestId employeeMeasurements registeredEmployeeIds poConversionDate",
      )
      .sort({ createdAt: -1 })
      .lean();
...
router.post("/:measurementId/assign-product", async (req, res) => {
  try {
    ...
    const { employeeId, organizationId, product, measurements, categoryName } =
      req.body;
    ...
    const alreadyAssigned = employeeDoc.products?.some((p) => {
      ...
      if (product.variantId)
        return p.variantId?.toString() === product.variantId.toString();
      return !p.variantId;
    });
...
router.post("/:measurementId/po-confirmation", async (req, res) => {
  try {
    ...
    const allEmpIds = measurement.employeeMeasurements
      .map((e) => e.employeeId)
      .filter(Boolean);
    ...
    mpcEmployees.forEach((emp) => {
      ...
      (emp.products || []).forEach((p) => {
        ...
        if (pid && p.productName?.trim())
          prodMap.set(pid, p.productName.trim());
      });
    ...
    for (const emp of measurement.employeeMeasurements || []) {
      ...
      for (const p of emp.products || []) {
        ...
        if (p.variantId && si.variants?.length) {
          const variant = si.variants.find(
...
router.post("/:measurementId/convert-to-po", async (req, res) => {
  try {
    ...
    const measurement = await Measurement.findById(measurementId)
      .populate({
        path: "employeeMeasurements.products.productId",
        select: "_id name reference baseSalesPrice variants",
      })
      .lean();
    ...
    if (measurement.convertedToPO)
      return res
        .status(400)
        .json({ success: false, message: "Already converted to PO" });

    const selectedSet = new Set(selectedEmployeeIds.map((id) => id.toString()));
    const excludedSet = new Set(excludedProductKeys);

    const selectedEmployeeMeasurements =
      measurement.employeeMeasurements.filter((e) =>
        selectedSet.has(e.employeeId?.toString()),
      );
    const remainingEmployeeMeasurements =
      measurement.employeeMeasurements.filter(
        (e) => !selectedSet.has(e.employeeId?.toString()),
      );
    ...
    const convertibleEmployees = splitEmployees.filter(
      (e) => e.includedProducts.length > 0,
    );

    // Employees whose excluded products need to go to the new measurement
    const employeesWithExcluded = splitEmployees.filter(
      (e) => e.excludedProducts.length > 0,
    );
    ...
    if (!convertibleEmployees.length) {
      return res.status(400).json({
        success: false,
        message:
          "All selected products are excluded. Include at least one product to create a PO.",
      });
    }
    ...
    for (const emp of convertibleEmployees) {
      for (const measuredProduct of emp.includedProducts) {
        ...
        if (needsAutoAssign && si.variants?.length > 0) {
          ...
          finalVariantName = dv.attributes?.length
            ? dv.attributes.map((a) => a.value).join(" • ")
            : dv.name || "Default";
          finalVariantAttributes =
            dv.attributes?.map((a) => ({
          ...
          const variantData = si.variants.find(
            (v) => v._id.toString() === finalVariantId.toString(),
          );
          if (variantData) {
      ...
      variants: [
        {
          ...
          attributes: p.variantAttributes.length
            ? p.variantAttributes
            : p.variantName !== "Default"
              ? [{ name: "Variant", value: p.variantName }]
              : [],
    ...
    const newRequest = new CustomerRequest({
      ...
      customerInfo: {
        ...
        description:
          poDescription || `PO from measurement: ${measurement.name}`,
    ...
    const excludedProductEntriesForNewMeasurement = employeesWithExcluded.map(
      (emp) => ({
        ...emp,
    ...
    if (allRemainingEntries.length > 0) {
      ...
      const remCompleted = allRemainingEntries.filter(
        (e) => e.isCompleted,
      ).length;
      const newMeasurementDoc = new Measurement({
        ...
        completionRate: Math.round(
          (remCompleted / allRemainingEntries.length) * 100,
        ),
        convertedToPO: false,
    ...
    const selCompleted = selEmpForPoMeasurement.filter(
      (e) => e.isCompleted,
    ).length;
    await Measurement.findByIdAndUpdate(measurementId, {
      ...
      completionRate:
        selEmpForPoMeasurement.length > 0
          ? Math.round((selCompleted / selEmpForPoMeasurement.length) * 100)
          : 100,
      convertedToPO: true,
    ...
    res.status(201).json({
      ...
      totalEstimatedPrice: validatedItems.reduce(
        (s, i) => s + i.totalEstimatedPrice,
        0,
      ),
      totalItems: validatedItems.length,
      selectedEmployeeCount: convertibleEmployees.length,
      remainingEmployeeCount: allRemainingEntries.length,
      excludedProductEmployeeCount:
        excludedProductEntriesForNewMeasurement.length,
      newMeasurementId,
      autoAssignedVariants:
        autoAssignedLog.length > 0 ? autoAssignedLog : undefined,
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Server error while converting to PO" });
  }
});
...
router.get("/organization/:orgId/export-product-pricing", async (req, res) => {
  try {
    ...
    if (!measurements?.length)
      return res
        .status(404)
        .json({ success: false, message: "No measurements found" });

    const measuredEmployeeIds = new Set();
    measurements.forEach((m) =>
      m.employeeMeasurements?.forEach((e) => {
        if (e.employeeId) measuredEmployeeIds.add(e.employeeId.toString());
      }),
    );
      ...
      (emp.products || []).forEach((pa) => {
        ...
        if (!productMap.has(key)) {
          ...
          if (pa.variantId && si.variants) {
            const v = si.variants.find(
              (v) => v._id.toString() === pa.variantId.toString(),
            );
            if (v) {
              variantName =
                v.attributes?.map((a) => a.value).join(" • ") || "Default";
              variantPrice = v.salesPrice;
            }
          }
          productMap.set(key, {
            productName: si.name,
    ...
    if (!products.length)
      return res
        .status(404)
        .json({ success: false, message: "No product assignments found" });
    ...
    const rows = products.map((p) => {
      ...
      return [
        ...
        p.totalQuantity,
        p.employeeCount,
        `"${Array.from(p.employeeNames).join(", ")}"`,
    ...
    const metadata = [
      ...
      `Total Measured Employees: ${allEmpNames.size}`,
    ...
    const csvContent = [
      ...
      headers.join(","),
      ...rows.map((r) => r.join(",")),
      `,,,,${grandQty},,,"Grand Total: ${grandTotal.toFixed(2)}"`,
    ].join("\n");

    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="Product_Pricing_${customer.name.replace(/\s+/g, "_")}_${new Date().toISOString().split("T")[0]}.csv"`,
    );
    res.send(csvContent);
  } catch (error) {
...
router.get("/:measurementId/export", async (req, res) => {
  try {
    ...
    measurement.employeeMeasurements.forEach((emp) => {
      ...
      (emp.products || []).forEach((p) => {
        ...
        (p.measurements || []).forEach((m) => {
          if (
            m.measurementName &&
            !categoryFieldsMap.get(cat).includes(m.measurementName)
          )
    ...
    const safeFileName = measurement.name
      .replace(/\s+/g, "_")
    ...
    res
      .status(500)
      .json({ success: false, message: "Server error while exporting" });
  }
});
...
router.get("/:measurementId/export-grouped", async (req, res) => {
  try {
      ...
      (emp.products || []).forEach((p) => {
        ...
        if (!productMap.has(key)) {
          productMap.set(key, {
            ...
            reference: p.productId?.reference || "",
            employees: [],
            measurementNames:
              p.measurements?.map((m) => m.measurementName) || [],
          });
        }
        const pd = productMap.get(key);
        const mValues = {};
        p.measurements?.forEach((m) => {
          mValues[m.measurementName] = m.value || "";
        });
        pd.employees.push({
          name: emp.employeeName,
          uin: emp.employeeUIN,
      ...
      pd.employees.forEach((e) => {
        csv +=
          [
            ...
            e.quantity,
            `"${e.remarks}"`,
            ...pd.measurementNames.map((n) => e.measurements[n] || ""),
          ].join(",") + "\n";
      });
      csv += "\n\n";
    });
    ...
    const catEmployees = measurement.employeeMeasurements.filter(
      (e) => e.noProductAssigned,
    );
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${measurement.name.replace(/\s+/g, "_")}_grouped.csv"`,
    );
    res.send(csv);
  } catch (error) {
