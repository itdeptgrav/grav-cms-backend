...
const findVariantNickname = (rawItemDoc, variantId, vendorId) => {
  if (!rawItemDoc || !vendorId || !Array.isArray(rawItemDoc.variants)) return "";
  if (variantId) {
    const variant = rawItemDoc.variants.find(v => v._id?.toString() === variantId.toString());
    if (variant?.vendorNicknames?.length) {
      const vn = variant.vendorNicknames.find(n => n.vendor?.toString() === vendorId.toString());
...
router.get("/", async (req, res) => {
  try {
    ...
    let totalAmount = 0, totalPaid = 0, pendingAmount = 0;
    purchaseOrders.forEach(po => {
      totalAmount  += po.totalAmount || 0;
      const poPaid  = (po.payments || []).reduce((s, p) => s + (p.amount || 0), 0);
      totalPaid    += poPaid;
    ...
    res.json({
      success: true, purchaseOrders,
      stats: { total, draft, issued, partiallyReceived, completed,
               totalAmount, totalPaid, pendingAmount,
               paymentPending, paymentPartial, paymentCompleted }
    });
  } catch (error) {
...
// ─────────────────────────────────────────────────────────────────────────────
// GET vendor items (with alias data)
// ─────────────────────────────────────────────────────────────────────────────
router.get("/data/vendor-items/:vendorId", async (req, res) => {
  try {
    ...
    const items = await RawItem.find({ "variants.vendorNicknames.vendor": vendorId })
      .select("name sku category customCategory unit customUnit description quantity minStock maxStock status variants unitConversion")
      .lean().sort({ name: 1 });

    const formatted = items.map(item => {
      const matchingVariants = (item.variants || [])
        .map(v => {
          const alias = (v.vendorNicknames || []).find(vn => vn.vendor?.toString() === vendorId);
  ...
  try {
    ...
    const formatted = rawItems.map(item => ({
      ...
      variants: (item.variants || []).map(v => ({
        _id: v._id.toString(), combination: v.combination || [],
        sku: v.sku || "", quantity: v.quantity || 0,
        image: v.image || "", aliasId: "", vendorCode: "", price: 0, deliveryDays: 0,
      })),
    }));
...
router.get("/data/raw-items/:id/units", async (req, res) => {
  try {
    const rawItem = await RawItem.findById(req.params.id).select("unit customUnit name").lean();
    ...
    const available = [{ name: baseUnit, isBase: true, factor: 1, label: `${baseUnit} (registered)` }];
    ...
    if (baseDoc?.conversions?.length) {
      for (const c of baseDoc.conversions) {
        ...
        if (toName && !available.find(u => u.name === toName))
          available.push({ name: toName, isBase: false, factor: c.quantity, label: `${toName} (1 ${baseUnit} = ${c.quantity} ${toName})` });
      }
    }
    if (baseDoc?._id) {
      const reverseUnits = await Unit.find({ "conversions.toUnit": baseDoc._id }).lean();
      for (const u of reverseUnits) {
        if (available.find(au => au.name === u.name)) continue;
        const conv = (u.conversions || []).find(c => c.toUnit?.toString() === baseDoc._id.toString());
        if (conv?.quantity)
          available.push({ name: u.name, isBase: false, factor: 1 / conv.quantity, label: `${u.name} (1 ${u.name} = ${conv.quantity} ${baseUnit})` });
      }
    }
  ...
  try {
    ...
    res.json({
      ...
      rawItems: rawItems.map(item => ({
        ...
        category: item.customCategory || item.category || "Uncategorized",
        unit: item.customUnit || item.unit || "unit",
        description: item.description || "", sellingPrice: item.sellingPrice || 0,
        currentStock: item.quantity || 0, minStock: item.minStock || 0,
        maxStock: item.maxStock || 0, status: item.status || "In Stock"
      }))
    });
  } catch (error) {
  ...
  try {
    ...
    res.json({
      ...
      vendors: vendors.map(v => ({
        id: v._id, name: v.companyName, contactPerson: v.contactPerson,
        phone: v.phone, email: v.email, address: v.address,
        gstNumber: v.gstNumber, vendorType: v.vendorType, paymentTerms: v.paymentTerms
      }))
    });
  } catch (error) {
...
router.post("/", async (req, res) => {
  try {
    const {
      vendor, vendorName, orderDate, expectedDeliveryDate,
      items, shippingCharges, discount,
      notes, termsConditions, paymentTerms, status = "DRAFT",
      piInvoiceNumber = "", piInvoicePhoto = ""
    } = req.body;
    ...
    let poNumber, isUnique = false, attempts = 0;
    ...
    if (!isUnique) return res.status(500).json({ success: false, message: "Failed to generate unique PO number" });

    const itemsWithDetails = await Promise.all(items.map(item => buildPoItem(item, vendor)));

    const subtotal  = itemsWithDetails.reduce((s, i) => s + (i.totalPrice  || 0), 0);
    const taxAmount = itemsWithDetails.reduce((s, i) => s + (i.gstAmount   || 0), 0);
    const totalAmount = subtotal + taxAmount
      + (Number(shippingCharges) || 0)
      - (Number(discount) || 0);

    const purchaseOrderData = {
      poNumber, vendor, vendorName: vendorName || "",
      orderDate: orderDate ? new Date(orderDate) : new Date(),
      expectedDeliveryDate: expectedDeliveryDate ? new Date(expectedDeliveryDate) : null,
      piInvoiceNumber: piInvoiceNumber || "",
      piInvoicePhoto:  piInvoicePhoto  || "",
      items: itemsWithDetails,
      subtotal, taxRate: 0, taxAmount,
      shippingCharges: Number(shippingCharges) || 0,
      discount: Number(discount) || 0,
      totalAmount,
      totalReceived: 0,
      totalPending: itemsWithDetails.reduce((s, i) => s + i.quantity, 0),
    ...
    if (status === "ISSUED") {
      try {
        const vendorData = await Vendor.findById(vendor).select("companyName contactPerson email phone address");
        if (vendorData?.email)
          await VendorEmailService.sendPurchaseOrderEmail(
            populatedPO.toObject(), vendorData.toObject(),
            { name: req.user.name || "Project Manager", email: req.user.email || "admin@example.com" }
          );
...
router.put("/:id", async (req, res) => {
  try {
    const {
      vendor, vendorName, orderDate, expectedDeliveryDate,
      items, shippingCharges, discount,
      notes, termsConditions, paymentTerms, status,
      piInvoiceNumber, piInvoicePhoto
    } = req.body;

    const purchaseOrder = await PurchaseOrder.findById(req.params.id);
    if (!purchaseOrder) return res.status(404).json({ success: false, message: "Purchase order not found" });
    if (["PARTIALLY_RECEIVED", "COMPLETED"].includes(purchaseOrder.status))
      return res.status(400).json({ success: false, message: "Cannot edit a PO that has already been received" });
    ...
    if (piInvoiceNumber !== undefined) purchaseOrder.piInvoiceNumber = piInvoiceNumber || "";
    if (piInvoicePhoto  !== undefined) purchaseOrder.piInvoicePhoto  = piInvoicePhoto  || "";

    if (items && Array.isArray(items)) {
      for (const item of items) {
        if (!item.rawItem)              return res.status(400).json({ success: false, message: "Raw item required for all items" });
        if (!item.quantity   || item.quantity   <= 0) return res.status(400).json({ success: false, message: "Valid quantity required" });
        if (!item.unitPrice  || item.unitPrice  <= 0) return res.status(400).json({ success: false, message: "Valid unit price required" });
      }

      const vendorId = purchaseOrder.vendor?.toString() || vendor;
      const itemsWithDetails = await Promise.all(items.map(item => buildPoItem(item, vendorId)));
      ...
      purchaseOrder.totalPending = itemsWithDetails.reduce((s, i) => s + i.quantity, 0);
    }
    ...
    res.json({ success: true, message: "Purchase order updated successfully", purchaseOrder: populatedPO });
  } catch (error) {
...
router.post("/:id/payment", async (req, res) => {
  try {
    ...
    if (!amount || amount <= 0) return res.status(400).json({ success: false, message: "Valid payment amount is required" });
    if (!paymentMethod)         return res.status(400).json({ success: false, message: "Payment method is required" });

    const purchaseOrder = await PurchaseOrder.findById(req.params.id);
    if (!purchaseOrder) return res.status(404).json({ success: false, message: "Purchase order not found" });

    const totalPaid      = (purchaseOrder.payments || []).reduce((s, p) => s + (p.amount || 0), 0);
    const remainingAmount = purchaseOrder.totalAmount - totalPaid;
    if (amount > remainingAmount)
      return res.status(400).json({ success: false, message: `Amount exceeds remaining ₹${remainingAmount}` });
    ...
    purchaseOrder.paymentStatus =
      newTotalPaid >= purchaseOrder.totalAmount ? "COMPLETED" :
      newTotalPaid > 0 ? "PARTIAL" : "PENDING";

    await purchaseOrder.save();
    const populatedPO = await PurchaseOrder.findById(purchaseOrder._id).populate("payments.recordedBy", "name email");

    res.json({
      success: true, message: `Payment of ₹${amount} recorded`,
      payment: populatedPO.payments[0], paymentStatus: purchaseOrder.paymentStatus,
      totalPaid: newTotalPaid, remainingAmount: purchaseOrder.totalAmount - newTotalPaid
    });
  } catch (error) {
...
router.get("/vendor/:vendorId", async (req, res) => {
  try {
    ...
    const purchaseOrders = await PurchaseOrder.find(filter)
      ...
      .populate("items.rawItem", "name sku").sort({ createdAt: -1 });
    const totalOrders  = purchaseOrders.length;
    const totalAmount  = purchaseOrders.reduce((s, po) => s + (po.totalAmount || 0), 0);
    const pendingAmount = purchaseOrders
      .filter(po => !["COMPLETED","CANCELLED"].includes(po.status))
      .reduce((s, po) => s + (po.totalAmount || 0), 0);
    res.json({ success: true, purchaseOrders, stats: { totalOrders, totalAmount, pendingAmount } });
  } catch (error) {
...
router.get("/raw-item/:itemId", async (req, res) => {
  try {
    ...
    const purchaseOrders = await PurchaseOrder.find(filter)
      ...
      .populate("vendor", "companyName").sort({ createdAt: -1 });
    res.json({
      success: true,
      purchaseOrders: purchaseOrders.map(po => {
        const item = po.items.find(i => i.rawItem.toString() === req.params.itemId);
        return {
          _id: po._id, poNumber: po.poNumber, orderDate: po.orderDate,
          expectedDeliveryDate: po.expectedDeliveryDate, vendorName: po.vendorName, status: po.status,
          itemDetails: item ? {
            quantity: item.quantity, receivedQuantity: item.receivedQuantity,
            pendingQuantity: item.pendingQuantity, unitPrice: item.unitPrice,
    ...
    console.error("Error fetching raw item POs:", error);
    res.status(500).json({ success: false, message: "Server error" });
  }
});
...
router.patch("/:id/status", async (req, res) => {
  try {
    ...
    if (purchaseOrder.totalReceived > 0 && status === "CANCELLED")
      return res.status(400).json({ success: false, message: "Cannot cancel a PO that has been received" });
    ...
    await purchaseOrder.save();
    res.json({ success: true, message: `Status updated to ${status}`, purchaseOrder });
  } catch (error) {
...
router.post("/:id/receive", async (req, res) => {
  try {
    ...
    for (const ri of items) {
      const poItem = purchaseOrder.items.find(it => it._id.toString() === ri.itemId?.toString());
      if (!poItem) return res.status(400).json({ success: false, message: `Item not found in PO: ${ri.itemId}` });
      const qty = parseFloat(ri.quantity) || 0;
      if (qty <= 0) continue;
      const pendingBefore = Math.max(0, +(poItem.quantity - poItem.receivedQuantity).toFixed(4));
      // Over-delivery is allowed — excess auto-credited to stock + StockIssuance record created
      updates.push({
        poItem,
        rawItemId: poItem.rawItem?._id || poItem.rawItem,
        variantId: ri.variantId || poItem.variantId,
        variantCombination: ri.variantCombination?.length ? ri.variantCombination : (poItem.variantCombination || []),
        qtyInPoUnit: qty,
        pendingBefore,
        poUnit: poItem.unit,
    ...
    if (!updates.length) return res.status(400).json({ success: false, message: "No valid quantities to receive" });
    ...
    for (const u of updates) {
      ...
      if (!rawItem) { console.warn(`RawItem not found: ${rawItemId}`); continue; }
      ...
      poItem.status = poItem.receivedQuantity >= poItem.quantity ? "COMPLETED"
      ...
      if (variantId) {
        ...
        for (let i = 0; i < rawItem.variants.length; i++) {
          if (rawItem.variants[i]._id?.toString() === variantId.toString()) { variant = rawItem.variants[i]; variantIdx = i; break; }
        }
      ...
      await rawItem.save();
      processed.push({
        itemName: poItem.itemName, variant: variantCombination?.join(" • ") || null,
        qtyInPoUnit, poUnit: fromUnit, qtyInRegisteredUnit, registeredUnit,
...
router.patch("/:id/payment-status", async (req, res) => {
  try {
    ...
    await purchaseOrder.save();
    res.json({ success: true, message: `Payment status updated to ${status}`, purchaseOrder });
  } catch (error) {
...
router.get("/:id/payments", async (req, res) => {
  try {
    const purchaseOrder = await PurchaseOrder.findById(req.params.id)
      ...
      .populate("payments.recordedBy", "name email");
    if (!purchaseOrder) return res.status(404).json({ success: false, message: "PO not found" });
    const totalPaid       = (purchaseOrder.payments || []).reduce((s, p) => s + p.amount, 0);
    const remainingAmount = purchaseOrder.totalAmount - totalPaid;
    res.json({
      success: true, payments: purchaseOrder.payments || [],
      totalPaid, remainingAmount, poNumber: purchaseOrder.poNumber, totalAmount: purchaseOrder.totalAmount
    });
  } catch (error) {
