...
router.get("/:id/with-details", async (req, res) => {
    try {
        ...
        const workOrder = await WorkOrder.findById(id)
            .populate('stockItemId', 'name reference numberOfPanels operations variants') // Add numberOfPanels
            .populate('customerRequestId', 'customerInfo requestId')
            .populate('createdBy', 'name email')
            .populate('plannedBy', 'name email')
    ...
    const rawMaterialsWithStock = await Promise.all(
      workOrder.rawMaterials.map(async (rm) => {
        if (rm.rawItemId) {
          ...
          if (rm.rawItemVariantId || (rm.rawItemVariantCombination && rm.rawItemVariantCombination.length > 0)) {
            if (rm.rawItemVariantId && rawItem?.variants) {
              const variant = rawItem.variants.find(v => 
                v._id.toString() === rm.rawItemVariantId.toString()
              );
              if (variant) {
                currentStock = variant.quantity || 0;
                maxUnitsFromThisMaterial = Math.floor(currentStock / requiredPerUnit);
              }
            } else if (rm.rawItemVariantCombination?.length > 0 && rawItem?.variants) {
              const variant = rawItem.variants.find(v => {
                if (!v.combination || v.combination.length !== rm.rawItemVariantCombination.length) {
                  return false;
                }
    ...
    try {
            ...
            if (rm.rawItemId) {
                ...
                if (rawItem) {
                    ...
                    if (rm.rawItemVariantId || (rm.rawItemVariantCombination && rm.rawItemVariantCombination.length > 0)) {
                        ...
                        // FIX: Check for division by zero
                        const maxUnitsFromThisVariant = requiredPerUnit > 0 ? Math.floor(variantStock / requiredPerUnit) : 0;
                        canProduceQuantity = Math.min(canProduceQuantity, maxUnitsFromThisVariant);
                    } else {
                        // No variant specified, use total stock
                        const maxUnitsFromThisMaterial = requiredPerUnit > 0 ? Math.floor(rawItem.quantity / requiredPerUnit) : 0;
                        canProduceQuantity = Math.min(canProduceQuantity, maxUnitsFromThisMaterial);
                    }
                }
            }
        }
        ...
        if (splitRemaining && remainingQuantity > 0) {
            ...
            const newWorkOrderData = {
                ...
                timeline: {
                    totalEstimatedSeconds: (workOrder.timeline?.totalEstimatedSeconds || 0) * (remainingQuantity / workOrder.quantity)
                },
        ...
        for (const rm of workOrder.rawMaterials) {
            ...
            const requiredPerUnit = workOrder.originalQuantity > 0 ? (rm.quantityRequired / workOrder.originalQuantity) : 0;
            
            // FIX: Ensure we have a valid number
            const newQuantityRequired = requiredPerUnit * quantity;
            rm.quantityRequired = isNaN(newQuantityRequired) ? 0 : newQuantityRequired;
...
router.put("/:id/plan-operations", async (req, res) => {
  try {
    ...
    if (totalPlannedSeconds) {
      const totalEstimated = workOrder.operations.reduce((total, op) => total + (op.estimatedTimeSeconds || 0), 0);
    ...
    for (const rawMaterial of workOrder.rawMaterials) {
      if (rawMaterial.rawItemId && rawMaterial.quantityAllocated > 0) {
        ...
        if (rawItem) {
          ...
          if (rawMaterial.rawItemVariantId || (rawMaterial.rawItemVariantCombination && rawMaterial.rawItemVariantCombination.length > 0)) {
            ...
            if (rawMaterial.rawItemVariantId && rawItem.variants) {
              ...
              variant = rawItem.variants.find(v => {
                if (!v.combination || v.combination.length !== rawMaterial.rawItemVariantCombination.length) {
                  return false;
                }
            ...
            if (variant) {
              ...
              variantNewQuantity = Math.max(0, variantPreviousQuantity - rawMaterial.quantityAllocated);
              variant.quantity = variantNewQuantity;
              
              // Also update total raw item quantity
              previousQuantity = rawItem.quantity;
              newQuantity = Math.max(0, rawItem.quantity - rawMaterial.quantityAllocated);
              rawItem.quantity = newQuantity;
              
              // FIXED: Use VARIANT_REDUCE instead of VARIANT_CONSUME
              transactionType = "VARIANT_REDUCE";
              variantInfo = rawMaterial.rawItemVariantCombination?.join(' • ') || `Variant ID: ${rawMaterial.rawItemVariantId?.toString().slice(-6)}`;
            } else {
              // Variant not found, fallback to total stock
              previousQuantity = rawItem.quantity;
              newQuantity = Math.max(0, previousQuantity - rawMaterial.quantityAllocated);
              rawItem.quantity = newQuantity;
