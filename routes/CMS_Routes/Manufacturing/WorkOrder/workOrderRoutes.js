...
router.get("/:id/planning", async (req, res) => {
  try {
    ...
    // Calculate maximum producible quantity based on raw material availability
    ...
    const rawMaterialsWithStock = await Promise.all(
      workOrder.rawMaterials.map(async (rm) => {
        if (rm.rawItemId) {
          ...
          const maxUnitsFromThisMaterial = Math.floor((rawItem?.quantity || 0) / requiredPerUnit);
          ...
          return {
            ...
            status: maxUnitsFromThisMaterial >= workOrder.quantity ? "sufficient" : 
                   maxUnitsFromThisMaterial > 0 ? "partial" : "insufficient"
          };
        }
        return rm;
      })
...
// UPDATE raw material allocation with quantity adjustment AND create new WO for remaining
router.put("/:id/allocate-raw-materials", async (req, res) => {
    try {
        ...
        // Calculate raw material requirements per unit and check stock
        ...
        for (const rm of workOrder.rawMaterials) {
            if (rm.rawItemId) {
                ...
                if (rawItem) {
                    const requiredPerUnit = rm.quantityRequired / workOrder.quantity;
                    const maxUnitsFromThisMaterial = Math.floor(rawItem.quantity / requiredPerUnit);
        ...
        // Update raw material quantities for current work order
        for (const rm of workOrder.rawMaterials) {
            const requiredPerUnit = rm.quantityRequired / workOrder.originalQuantity;
            rm.quantityRequired = requiredPerUnit * quantity;
            
            // Auto-allocate based on available stock
            if (rm.rawItemId) {
                const rawItem = await RawItem.findById(rm.rawItemId);
                if (rawItem) {
                    const maxAllocatable = Math.min(rm.quantityRequired, rawItem.quantity);
                    rm.quantityAllocated = maxAllocatable || 0;
...
// COMPLETE planning and issue raw materials
router.post("/:id/complete-planning", async (req, res) => {
  try {
    ...
    // Update raw material stock and create transactions
    ...
    for (const rawMaterial of workOrder.rawMaterials) {
      if (rawMaterial.rawItemId && rawMaterial.quantityAllocated > 0) {
        ...
        if (rawItem) {
          ...
          const newQuantity = previousQuantity - rawMaterial.quantityAllocated;
          
          rawItem.quantity = Math.max(0, newQuantity);
          ...
          rawItem.stockTransactions.push({
            ...
            notes: `Work Order: ${workOrder.workOrderNumber}, Product: ${workOrder.stockItemName}, Quantity: ${workOrder.quantity} units`,
