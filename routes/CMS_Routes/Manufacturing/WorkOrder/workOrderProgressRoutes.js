...
function parseBarcodeId(barcodeId) {
  try {
    ...
    const [, workOrderNumber, unitStr] = match;
    const unit = parseInt(unitStr);

    if (isNaN(unit)) {
      return { valid: false, error: "Invalid unit number" };
    }
...
router.get("/:id/progress", async (req, res) => {
  try {
    ...
    for (let unit = 1; unit <= totalUnits; unit++) {
      ...
      unitDetails.push({
        ...
        lastScannedOperation:
          scans.length > 0 ? scans[scans.length - 1].operation : null,
      });
    }
    ...
    const overallCompletionPercentage =
      totalUnits > 0 && totalOperations > 0
        ? Math.round((totalCompletedOperations / maxPossibleCompletedOps) * 100)
        : 0;

    // Ensure percentage doesn't exceed 100%
    const safeCompletionPercentage = Math.min(
      100,
      Math.max(0, overallCompletionPercentage),
    );
    ...
    const recentScans = workOrderScans
      ...
      .map((scan, index, array) => {
        ...
        return {
          ...
          timestamp: scan.timestamp,
          operationType:
            workOrder.operations[scan.operation - 1]?.operationType ||
            "Unknown",
          meaning: meaning,
        };
      });
    ...
    res.json({
      ...
      progress: {
        ...
        unitCompletionPercentage:
          totalUnits > 0
            ? Math.round((completedUnits.size / totalUnits) * 100)
            : 0,
        ...
        canComplete:
          ...
          completedUnits.size === totalUnits,
      },
      ...
      scanningLogic: {
        explanation:
          "Scan Logic: Each scan starts a new operation and completes the previous one",
...
router.get("/:id/unit/:unitNumber/timeline", async (req, res) => {
  try {
    ...
    scans.forEach((scan, index) => {
      ...
      if (index === 0) {
        ...
        timeline.push({
          ...
          operationType:
            workOrder.operations[previousOperation - 1]?.operationType ||
            "Unknown",
          timestamp: scan.timeStamp,
    ...
    if (scans.length > 0) {
      ...
      // Count completed
      completedOperations = Object.values(operationStatus).filter(
        (s) => s === "completed",
      ).length;
    }
    ...
    res.json({
      ...
      scans: scans.map((scan) => ({
        ...
        timestamp: scan.timeStamp,
        operationType:
          workOrder.operations[scan.operation - 1]?.operationType || "Unknown",
      })),
    });
  } catch (error) {
...
router.get("/:id/real-time-tracking", async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Get today's production tracking data
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const trackingDoc = await ProductionTracking.findOne({ date: today })
      .populate("machines.machineId", "name serialNumber")
      .populate({
        path: "machines.operators.operatorIdentityId",
        select: "firstName lastName identityId",
      });

    // Extract all barcode scans for today
    const allScans = [];

    if (trackingDoc && trackingDoc.machines) {
        ...
        machine.operators.forEach((operator) => {
          operator.barcodeScans.forEach((scan) => {
            allScans.push({
              ...
              operatorId: operator.operatorIdentityId?.identityId,
              operatorName: operator.operatorIdentityId
                ? `${operator.operatorIdentityId.firstName} ${operator.operatorIdentityId.lastName}`
                : "Unknown Operator",
            });
          });
        });
    ...
    // Parse barcodes and filter for current work order
    const workOrderShortId = workOrder._id.toString().slice(-8);
    const workOrderScans = [];
    const scannedUnits = new Set();

    allScans.forEach((scan) => {
      // Parse barcode format: WO-[MongoID]-[Unit3]
      const parts = scan.barcodeId.split("-");

      if (parts.length >= 3 && parts[0] === "WO") {
        const barcodeWorkOrderId = parts[1];
        const unitNumber = parts[2];

        // Check if this barcode belongs to current work order
        if (barcodeWorkOrderId === workOrderShortId) {
          const unitNum = parseInt(unitNumber);
          scannedUnits.add(unitNum);

          workOrderScans.push({
            ...scan,
            unitNumber: unitNum,
            operationNumber: 1, // Default operation (adjust based on your logic)
            operationType:
              workOrder.operations[0]?.operationType || "Operation",
          });
        }
      }
    });
    ...
    const totalUnits = workOrder.quantity;
    const totalOperations = workOrder.operations.length;
    const completedUnits = Array.from(scannedUnits).length;

    // Determine unit status
    const unitDetails = [];
    for (let unit = 1; unit <= totalUnits; unit++) {
      const isScanned = scannedUnits.has(unit);
      const unitScans = workOrderScans.filter(
        (scan) => scan.unitNumber === unit,
      );

      unitDetails.push({
        unitNumber: unit,
        status: isScanned ? "scanned" : "pending",
        scannedCount: unitScans.length,
        lastScan:
          unitScans.length > 0
            ? unitScans[unitScans.length - 1].timestamp
            : null,
        firstScan: unitScans.length > 0 ? unitScans[0].timestamp : null,
        machineUsed: unitScans.length > 0 ? unitScans[0].machineName : null,
        operatorUsed: unitScans.length > 0 ? unitScans[0].operatorName : null,
      });
    }
    ...
    res.json({
      ...
      workOrder: {
        id: workOrder._id,
        workOrderNumber: workOrder.workOrderNumber,
        shortId: workOrderShortId,
        productName: workOrder.stockItemName,
      },
      unitTracking: {
        totalUnits: totalUnits,
        scannedUnits: completedUnits,
      ...
      scanStatistics: {
        totalScansToday: allScans.length,
        workOrderScans: workOrderScans.length,
        averageScansPerUnit:
          completedUnits > 0
            ? (workOrderScans.length / completedUnits).toFixed(1)
            : 0,
      },
      barcodeFormat: {
        example: `WO-${workOrderShortId}-001`,
        explanation: "Format: WO-[WorkOrderShortId]-[UnitNumber]",
      },
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      message: "Server error while getting real-time tracking",
    });
  }
});
