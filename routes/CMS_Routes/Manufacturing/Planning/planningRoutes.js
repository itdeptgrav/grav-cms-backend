...
const calculateRawMaterialRequirements = async (stockItemId, quantity) => {
  const stockItem =
    await StockItem.findById(stockItemId).populate("rawItems.rawItemId");
  if (!stockItem) return [];
...
const getAvailableMachines = async (machineCategory = null) => {
  ...
  return await Machine.find(query)
    .select(
      "name type model status powerConsumption location lastMaintenance nextMaintenance",
    )
    .lean();
};
...
router.get("/:id", async (req, res) => {
  try {
    ...
    const planning = await Planning.findById(id)
      ...
      .populate(
        "machineAssignments.machines.machineId",
...
router.get("/work-order/:workOrderId/details", async (req, res) => {
  try {
    ...
    const workOrder = manufacturingOrder.workOrders.find(
...
router.post("/", async (req, res) => {
  try {
    ...
    if (planning) {
      ...
      planning.rawMaterialAssignments =
        rawMaterialAssignments || planning.rawMaterialAssignments;
      planning.machineAssignments =
        machineAssignments || planning.machineAssignments;
...
router.post("/:id/approve", async (req, res) => {
  try {
    ...
    if (
      !planning.progress.rawMaterialAssignment.completed ||
    ...
    const manufacturingOrder = await ManufacturingOrder.findById(
      planning.manufacturingOrderId,
    );
    if (manufacturingOrder) {
      const workOrder = manufacturingOrder.workOrders.find(
        (wo) => wo.workOrderId === planning.workOrderId,
      );
      if (workOrder) {
...
router.put("/:id/machines", async (req, res) => {
  try {
    ...
    if (
      !["draft", "raw_material_assigned", "machine_assigned"].includes(
        planning.status,
      )
    ) {
      return res.status(400).json({
