...
// ─── POST /signin  ─────────────────────────────────────────────────────────────
// QC person scans their employee ID card barcode (biometricId) to start a session.
router.post("/signin", async (req, res) => {
  try {
    ...
    const employee = await Employee.findOne({ biometricId: trimmedId })
      .select("firstName middleName lastName biometricId identityId department designation isActive status")
      .lean();

    if (!employee) {
      return res.status(404).json({
        success: false,
        message: "No employee found with this ID. Please check your ID card or contact your supervisor.",
      });
    }
    ...
    if (employee.isActive === false || employee.status === "inactive") {
      return res.status(403).json({
        success: false,
        message: "This employee account is inactive. Please contact your supervisor.",
      });
    }
  ...
  try {
    ...
    if (empProgress?.employeeName) {
      ...
      if (empProgress.employeeId) {
        empMpcDoc = await EmployeeMpc.findById(empProgress.employeeId)
          .select("department designation").lean().catch(() => null);
      }
      if (!empMpcDoc && empProgress.employeeUIN) {
        empMpcDoc = await EmployeeMpc.findOne({ uin: empProgress.employeeUIN.trim().toUpperCase() })
          .select("department designation").lean().catch(() => null);
      }
      ...
      if (measurementIdToUse) {
        try {
          const mDoc = await Measurement.findById(measurementIdToUse)
            .select("employeeMeasurements").lean();
          if (mDoc) {
            const empEntry = (mDoc.employeeMeasurements || []).find((em) =>
              (empProgress.employeeId && em.employeeId &&
               em.employeeId.toString() === empProgress.employeeId.toString()) ||
              (empProgress.employeeUIN && em.employeeUIN &&
               em.employeeUIN.trim().toUpperCase() === empProgress.employeeUIN.trim().toUpperCase()) ||
              em.employeeName === empProgress.employeeName
            );
            if (empEntry) {
              const prodEntry = (empEntry.products || []).find(
                (p) => p.productId?.toString() === workOrder.stockItemId?.toString()
              );
              if (prodEntry?.measurements?.length) {
                pieceOwner.measurements = prodEntry.measurements;
              }
            }
          }
        } catch (e) {
          console.warn("[QC lookup-piece] measurement fetch skipped:", e.message);
        }
      }
    }
    ...
    // ── Build operations list from master + attach scan operators ──────────────
...
router.post("/save-inspection", async (req, res) => {
  try {
    const {
      ...
      status, defects,
      qcSession,            // ← sent from frontend localStorage session
    } = req.body;
      ...
      ? defects
          ...
          .map((d) => ({
            ...
            operators: Array.isArray(d.operators)
              ? d.operators
                  .map((o) => ({ operatorId: o.operatorId || "", operatorName: o.operatorName || "" }))
    ...
    // ── Resolve QC person (localStorage session takes priority) ───────────────
    ...
    const record = await QCInspection.create({
      ...
      inspectedByQCName:      qcName,
      inspectedByBiometricId: qcBioId,
      inspectedByQCId:        qcId,
    });
...
router.get("/inspections", async (req, res) => {
  try {
    ...
    // Filter by QC person if provided
    ...
    const wos   = woIds.length
      ? await WorkOrder.find({ _id: { $in: woIds } })
          .select("workOrderNumber stockItemName stockItemReference variantAttributes stockItemId").lean()
      : [];
    ...
    enriched.forEach((i) => {
      // tally by QC person
      if (i.inspectedByQCName) {
        byQCPerson[i.inspectedByQCName] = (byQCPerson[i.inspectedByQCName] || 0) + 1;
      }
      (i.defects || []).forEach((d) => {
    ...
    res.json({
      ...
      passed, defective, totalDefectOps,
