// routes/Accountant_Routes/Acc_bankTransactions.js
//
...
// Acc_Voucher entries within ±3 days with the same amount.
  ...
  try {
    ...
    const results = await Promise.all(
      banks.map(async (acc, idx) => {
        ...
        const stat = await Acc_BankTransaction.aggregate([
...
router.post("/", async (req, res) => {
  try {
    ...
    const txn = await Acc_BankTransaction.create(data);
...
router.get("/:id/auto-match-suggestions", async (req, res) => {
  try {
    const txn = await Acc_BankTransaction.findById(req.params.id).lean();
    if (!txn)
      return res
        .status(404)
...
router.post("/:id/reconcile", async (req, res) => {
  try {
    const txn = await Acc_BankTransaction.findById(req.params.id);
    if (!txn)
      return res
        .status(404)
        .json({ success: false, message: "Transaction not found" });
...
router.post("/:id/unreconcile", async (req, res) => {
  try {
    await Acc_BankTransaction.findByIdAndUpdate(req.params.id, {
      isReconciled: false,
      reconciledWith: null,
      reconciledBy: null,
      reconciledAt: null,
...
router.delete("/:id", async (req, res) => {
  try {
    await Acc_BankTransaction.findByIdAndDelete(req.params.id);
    res.json({ success: true });
  } catch (error) {
    res
      .status(500)
...
router.get("/accounts/summary", async (req, res) => {
  try {
    const accounts = await Acc_BankTransaction.aggregate([
      { $sort: { transactionDate: -1 } },
      {
        $group: {
          _id: "$bankAccount",
