// routes/Accountant_Routes/proformaInvoices.js
// =============================================================================
...
const {
  ProformaInvoice,
} = require("../../models/Accountant_model/ProformaInvoiceModel");
const {
  TallyCompany,
  TallyLedger,
...
router.post("/", async (req, res) => {
  try {
    ...
    const company = await TallyCompany.findById(body.companyId).lean();
    if (!company) {
      return res
        .status(404)
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (Array.isArray(body.items)) {
      ...
      // POST handler above.
      const company = await TallyCompany.findById(pi.companyId).lean();
      const sellerStateCode = company?.address?.stateCode || "";
