...
router.get("/", async (req, res) => {
  try {
    ...
    const allFilings = await TaxFiling.find(
...
router.put("/:id", async (req, res) => {
  try {
    const filing = await TaxFiling.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
    if (!filing)
      return res
...
router.delete("/:id", async (req, res) => {
  try {
    const filing = await TaxFiling.findByIdAndDelete(req.params.id);
    if (!filing)
      return res
        .status(404)
        .json({ success: false, message: "Filing not found" });
