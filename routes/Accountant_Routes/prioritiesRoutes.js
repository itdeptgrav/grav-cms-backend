// routes/Accountant_Routes/prioritiesRoutes.js
//
...
const RowPriority = require("../../models/Accountant_model/RowPriority");
...
router.get("/", async (req, res) => {
  try {
    ...
    const items = await RowPriority.find({ userId: userId(req), entityType })
      .sort({ rank: 1 })
...
router.put("/", async (req, res) => {
  try {
    ...
    const result = await RowPriority.bulkWrite(ops, { ordered: false });
    res.json({
      success: true,
...
router.delete("/:entityType/:entityId", async (req, res) => {
  try {
    ...
    const out = await RowPriority.deleteOne({
      userId: userId(req),
      entityType,
      entityId,
    });
...
router.delete("/:entityType", async (req, res) => {
  try {
    if (!requireRealUser(req, res)) return;
    const out = await RowPriority.deleteMany({
      userId: userId(req),
