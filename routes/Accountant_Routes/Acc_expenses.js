// routes/Accountant_Routes/Acc_expenses.js
//
// EXPENSES — operational expense tracking that ACTUALLY POSTS to books.
//
// Architecture decision: an "expense" in our system is a Acc_Voucher
// of type "payment" or "journal" with `sourceSystem: "expense_module"`.
// We don't have a separate Acc_Expense collection — that would create data
...
// "AccountantModels.Acc_Expense" and crashed at module-load. Page was showing
...
async function applyLedgerBalances(voucher, direction = 1) {
  ...
  if (ops.length) await Acc_Ledger.bulkWrite(ops);
...
/* GET / — list expenses                                               */
/* ------------------------------------------------------------------ */
/* Returns vouchers with sourceSystem "expense_module" plus a
...
router.get("/", async (req, res) => {
  try {
    ...
    // Summary KPIs
    const allForSummary = await Acc_Voucher.find(filter)
...
router.post("/", async (req, res) => {
  try {
    ...
    const expenseLg = await Acc_Ledger.findById(expenseLedgerId);
    if (!expenseLg)
      return res.status(400).json({ error: "Acc_Expense ledger not found" });
    ...
    if (mode === "pay_now") {
      counterLg = await Acc_Ledger.findById(bankLedgerId);
      if (!counterLg)
        return res.status(400).json({ error: "Bank ledger not found" });
    } else {
      counterLg = await Acc_Ledger.findById(vendorLedgerId);
      if (!counterLg)
    ...
    if (gstApplicable) {
      ...
      const gstRows = await Acc_Ledger.find({
        companyId,
        isActive: true,
        name: {
    ...
    // Dr: Acc_Expense ledger (taxable value)
    ledgerEntries.push({
...
router.post("/:id/approve", async (req, res) => {
  try {
    ...
    if (!v) return res.status(404).json({ error: "Acc_Expense not found" });
...
router.get("/:id", async (req, res) => {
  try {
    const v = await Acc_Voucher.findById(req.params.id).lean();
    if (!v) return res.status(404).json({ error: "Acc_Expense not found" });
    res.json({ expense: v });
  } catch (e) {
    res.status(500).json({ error: e.message });
