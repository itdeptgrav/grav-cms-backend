...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (updates.isPrimary) {
      await Acc_Company.updateMany({ _id: { $ne: req.params.id }, isPrimary: true }, { isPrimary: false });
    }

    const company = await Acc_Company.findByIdAndUpdate(req.params.id, updates, { new: true });
    if (!company) return res.status(404).json({ success: false, message: "Company not found" });
    res.json({ success: true, company });
  } catch (err) {
    res.status(500).json({ success: false, message: err.message });
...
router.post("/:id/reseed-groups", async (req, res) => {
  try {
    ...
    const existing = await Acc_Group.countDocuments({ companyId: company._id, isReserved: true });
