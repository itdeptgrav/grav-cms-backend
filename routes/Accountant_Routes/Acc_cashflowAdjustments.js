// routes/Accountant_Routes/Acc_cashflowAdjustments.js
//
...
const Acc_CashFlowAdjustment = require("../../models/Accountant_model/Acc_CashFlowAdjustment");
...
router.get("/", async (req, res) => {
  try {
    ...
    const rows = await Acc_CashFlowAdjustment.find(q)
      .sort({ createdAt: -1 })
      .lean();
...
router.get("/pending", async (req, res) => {
  try {
    ...
    const rows = await Acc_CashFlowAdjustment.find({
...
router.post("/", async (req, res) => {
  try {
    ...
    const doc = new Acc_CashFlowAdjustment({
      organizationId: req.user.organizationId,
      section,
      label: label.trim(),
      amount: amt,
...
router.put("/:id", async (req, res) => {
  try {
    ...
    const doc = await Acc_CashFlowAdjustment.findOne({
      _id: id,
      organizationId: req.user.organizationId,
    });
    if (!doc)
