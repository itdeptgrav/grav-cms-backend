// routes/Accountant_Routes/setuAA.js
//
...
//
// Connector state lives in the SetuConsent collection (one doc per bank
// account) — we don't try to stuff it into AccountantSettings, which has
...
const SetuConsent = require("../../models/Accountant_model/SetuConsent");
...
router.post("/webhook", async (req, res) => {
  try {
    ...
    if (type === "CONSENT_STATUS_UPDATE") {
      ...
      await SetuConsent.findOneAndUpdate(
        { consentId },
        {
          status:
...
router.get("/status", async (req, res) => {
  try {
    ...
    const consents = await SetuConsent.find().lean();
    res.json({
      success: true,
      configured,
...
router.post("/init-consent", async (req, res) => {
  try {
    ...
    // Upsert into SetuConsent collection — one doc per bank account
    const saved = await SetuConsent.findOneAndUpdate(
      { bankAccount },
      {
        bankAccount,
        consentId,
...
router.post("/sync/:bankAccount", async (req, res) => {
  try {
    ...
    let consent = await SetuConsent.findOne({ bankAccount });
    if (!consent && bodyConsentId) {
      consent = await SetuConsent.findOne({ consentId: bodyConsentId });
    }

    if (!consent || !consent.consentId) {
      const all = await SetuConsent.find().lean();
      console.warn(
