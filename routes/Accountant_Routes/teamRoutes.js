// routes/Accountant_Routes/teamRoutes.js
//
...
router.get("/", async (req, res) => {
  try {
    ...
    const users = await AccountantUser.find({
...
router.get("/invites", async (req, res) => {
  try {
    ...
    const invites = await AccountantInvite.find({
      organizationId: req.user.organizationId,
      consumedAt: null,
    })
      .sort({ createdAt: -1 })
...
router.post("/invites", requireRole("owner"), async (req, res) => {
  try {
    ...
    const existingUser = await AccountantUser.findOne({
      organizationId: req.user.organizationId,
      email: lowerEmail,
    });
    if (existingUser) {
    ...
    const liveInvite = await AccountantInvite.findOne({
      organizationId: req.user.organizationId,
      email: lowerEmail,
      consumedAt: null,
    ...
    const invite = await AccountantInvite.create({
      organizationId: req.user.organizationId,
      name,
      email: lowerEmail,
      role,
...
router.delete("/invites/:id", requireRole("owner"), async (req, res) => {
  try {
    const invite = await AccountantInvite.findOne({
      _id: req.params.id,
      organizationId: req.user.organizationId,
    });
    if (!invite)
...
router.put("/:userId/nav-prefs", requireRole("owner"), async (req, res) => {
  try {
    ...
    const target = await AccountantUser.findOne({
      _id: req.params.userId,
      organizationId: req.user.organizationId,
    });
    if (!target)
