// routes/Accountant_Routes/Acc_dashboard.js
...
router.get("/", async (req, res) => {
  try {
    ...
    // ── Acc_Expense breakdown by category ──
    ...
    // ── Invoices summary ──
    const invoices = await Acc_Invoice.find({ createdAt: { $gte: startOfYear } })
    ...
    // ── Tax obligations ──
    const upcomingTaxes = await Acc_TaxFiling.find({
    ...
    // ── Acc_Budget status ──
    const activeBudget = await Acc_Budget.findOne({ status: "active" })
      .sort({ createdAt: -1 })
      .lean();
    ...
    // the older Acc_Department auth (not the org-role auth).
    //
    ...
    const pendingApprovalsRaw = await Acc_ApprovalRequest.find({
      ...orgScope,
      status: "pending",
    })
      .sort({ createdAt: -1 })
    ...
    const pendingApprovalsCount = await Acc_ApprovalRequest.countDocuments({
      ...orgScope,
      status: "pending",
    });
    ...
    let mySubmissions = [];
    if (req.user?.id) {
      mySubmissions = await Acc_ApprovalRequest.find({
        ...orgScope,
    ...
    const recentApprovalEvents = await Acc_ApprovalRequest.find({
      ...orgScope,
...
router.get("/balance-sheet", async (req, res) => {
  try {
    ...
    // Bank balances
    const bankTxns = await Acc_BankTransaction.find()
    ...
    // Tax payable
    const pendingTaxes = await Acc_TaxFiling.find({
...
router.get("/cash-flow", async (req, res) => {
  try {
    ...
    for (let i = parseInt(months) - 1; i >= 0; i--) {
      ...
      // Outflows
      const monthExpenses = await Acc_Expense.find({
        createdAt: { $gte: monthStart, $lte: monthEnd },
        status: { $ne: "void" },
      })
        .select("totalAmount")
