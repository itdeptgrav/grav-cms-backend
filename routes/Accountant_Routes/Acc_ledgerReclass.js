...
//            Applies the change to Acc_Ledger.
//
...
// Apply a request: mutate the underlying Acc_Ledger. Idempotent — if
...
async function applyRequest(req, reqDoc) {
  ...
  const destGroup = await Acc_Group.findOne({
...
router.post("/propose", async (req, res) => {
  try {
    ...
    const fromGroup = await Acc_Group.findById(ledger.groupId).lean();
    const toGroup = await Acc_Group.findOne({
      _id: toGroupId,
      companyId,
      isActive: true,
    }).lean();
    ...
    const reqDoc = new Acc_LedgerReclassRequest({
      organizationId: req.user.organizationId,
      companyId,
      ledgerId,
...
router.post("/:id/approve", async (req, res) => {
  try {
    ...
    const reqDoc = await Acc_LedgerReclassRequest.findOne({
      _id: req.params.id,
      organizationId: req.user.organizationId,
      status: "pending_approval",
    });
