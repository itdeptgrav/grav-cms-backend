...
router.get("/cash-bank-ledgers", auth, async (req, res) => {
  try {
    ...
    const rootGroups = await TallyGroup.find({
      companyId,
      isActive: true,
...
async function resolveSalesReturnsLedger(companyId) {
  ...
  let group = await TallyGroup.findOne({
    companyId,
    isActive: true,
    name: /^sales\s*accounts$/i,
  });
  if (!group) {
    group = await TallyGroup.findOne({
      companyId,
      isActive: true,
      nature: "revenue",
    });
  ...
  const led = await TallyLedger.create({
    companyId,
    name: "Sales Returns",
    groupId: group._id,
...
router.get("/invoice-lookup", auth, async (req, res) => {
  try {
    ...
    const invoices = await TallyVoucher.find({
      companyId,
      voucherType: "sales",
      status: "posted",
      partyLedgerId,
    ...
    const priorCNs = await TallyVoucher.aggregate([
      {
        $match: {
...
router.get("/bill-lookup", auth, async (req, res) => {
  try {
    ...
    const bills = await TallyVoucher.find(filter)
      .sort({ voucherDate: -1 })
...
router.get("/unpaid-invoices", auth, async (req, res) => {
  try {
    ...
    const priorReceipts = await TallyVoucher.find({
      companyId,
      voucherType: "receipt",
      status: "posted",
      partyLedgerId,
...
router.get("/:id", auth, async (req, res) => {
  try {
    ...
    let voucher;
    try {
      voucher = await TallyVoucher.findById(req.params.id)
      ...
      voucher = await TallyVoucher.findById(req.params.id).lean();
    }
  ...
  try {
    ...
    if (partyId) {
      ...
      if (!body.partyLedgerName) {
        const led = await TallyLedger.findById(partyId).select("name");
    ...
    for (const entry of body.ledgerEntries) {
      ...
      if (ledId) {
        ...
        if (!entry.ledgerName) {
          const led = await TallyLedger.findById(ledId).select("name");
