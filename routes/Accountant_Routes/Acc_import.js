...
router.post("/sessions/:id/rollback", async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      message: `Rolled back ${deleted} records`,
...
router.post("/bsheet/commit", async (req, res) => {
  try {
    ...
    // ── Pass 1: ensure every needed group exists ──────────────────────
    // The seed-manufacturing endpoint creates the 28 default groups.
    // If the user hasn't run that, we top-up here so the import doesn't
    // fail with "group not found".
    const groupCache = new Map();
    async function getOrCreateGroup(name, nature) {
      const key = name.trim().toLowerCase();
      if (groupCache.has(key)) return groupCache.get(key);
      let g = await Acc_Group.findOne({ companyId, name });
      if (!g) {
    ...
    for (const l of parsed.ledgers) {
      try {
        ...
        const group = await getOrCreateGroup(groupName, ov.nature || l.nature);
        ...
        const openingBalanceType =
          ov.openingBalanceType || l.openingBalanceType;
        ...
        if (existing) {
          ledgerByName.set(l.name.trim().toLowerCase(), existing);
          skipped.ledgers++;
          continue;
        }
        ...
        const led = await Acc_Ledger.create({
          ...
          contactDetails: l.address
            ...
            : undefined,
          importSource: "tally_csv", // closest match in enum; "tally_json" not in schema yet
          importedAt: new Date(),
          createdBy: accountantId,
        });
        ledgerByName.set(l.name.trim().toLowerCase(), led);
        created.ledgers++;
        rollbackTokens.push({ collection: "acc_ledgers", objectId: led._id });
      } catch (e) {
        errors.push({ item: l.name, kind: "ledger", reason: e.message });
      }
    }

    // ── Pass 3: write vouchers ──────────────────────────────────────
    // A voucher needs its ledgerEntries[] to reference real Acc_Ledger
    // ObjectIds. If either ledger in a voucher isn't found, skip and
    // record the reason — the user can re-run after creating the
    // missing ledger.
    ...
    for (const v of parsed.vouchers) {
      try {
        ...
        for (const e of v.entries) {
          ...
          if (!led) {
            // The other side might be a ledger that wasn't in the export
            // (e.g. an income/expense account referenced only in passing).
            // Auto-create a minimal stub so the voucher can post.
            const stubGroup = await getOrCreateGroup(
              guessGroupForUnknownLedger(e.ledgerName),
              "asset",
            );
            const stub = await Acc_Ledger.create({
              companyId,
              name: e.ledgerName,
              groupId: stubGroup._id,
              groupName: stubGroup.name,
              nature: stubGroup.nature,
              openingBalance: 0,
              currentBalance: 0,
            ...
            ledgerByName.set(e.ledgerName.trim().toLowerCase(), stub);
            created.ledgers++;
            rollbackTokens.push({
              collection: "acc_ledgers",
              objectId: stub._id,
            ...
            ledgerDocs.push({ led: stub, side: e.side, amount: e.amount });
        ...
        // Sales / purchase / receipt / payment etc all need a party ledger.
        // Use the first non-bank-non-cash ledger as a best-guess party.
        let partyLedger = null;
        ...
        voucherSequence++;
        const vch = await Acc_Voucher.create({
          companyId,
          voucherType: v.voucherType,
          voucherTypeName: v.voucherType,
          voucherNumber:
            v.voucherNumber ||
            `IMP-${session._id.toString().slice(-6)}-${voucherSequence}`,
          voucherDate: v.date,
          partyLedgerId: partyLedger?._id,
        ...
        created.vouchers++;
        rollbackTokens.push({ collection: "acc_vouchers", objectId: vch._id });
      } catch (e) {
        errors.push({
