// routes/Accountant_Routes/Acc_pins.js
//
// PIN-TO-TOP API.
//
...
const Acc_PinnedItem = require("../../models/Accountant_model/Acc_PinnedItem");
...
// and require a real Acc_User._id to belong to.
...
router.get("/", async (req, res) => {
  try {
    ...
    const pins = await Acc_PinnedItem.find({
...
router.post("/", async (req, res) => {
  try {
    ...
    const pin = await Acc_PinnedItem.findOneAndUpdate(
      {
        userId: currentUserId(req),
    ...
    if (e?.code === 11000) {
      try {
        const existing = await Acc_PinnedItem.findOne({
          userId: currentUserId(req),
...
router.delete("/:entityType/:entityId", async (req, res) => {
  try {
    ...
    const out = await Acc_PinnedItem.deleteOne({
      userId: currentUserId(req),
      entityType,
      entityId,
    });
...
router.post("/reorder", async (req, res) => {
  try {
    ...
    const result = await Acc_PinnedItem.bulkWrite(ops);
