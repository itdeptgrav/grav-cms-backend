...
const { Acc_JournalEntry, ActivityLog } = require("../../models/Accountant_model/Acc_OperationalModels");
...
router.post("/:id/void", async (req, res) => {
  try {
    const entry = await Acc_JournalEntry.findByIdAndUpdate(req.params.id, { status: "void" }, { new: true });
    if (!entry) return res.status(404).json({ success: false, message: "Entry not found" });
