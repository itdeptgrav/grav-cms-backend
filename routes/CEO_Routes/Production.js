...
async function resolveEmployeeThreeStep(opId, Employee) {
  ...
  let emp = await Employee.findOne({ biometricId: opId })
    .select(
      "firstName lastName identityId biometricId profilePhoto department designation",
    )
    .lean();
  if (emp) return emp;
...
function buildEmpMap(employees) {
  ...
  for (const e of employees) {
    ...
    if (e.identityId && e.identityId !== e.biometricId)
      m.set(e.identityId, data);
  }
  return m;
}
...
function findIdlePeriods(scans, thresholdMins = 10) {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    if (fromH >= 12 && fromH <= 14 && gapMins >= 20 && gapMins <= 90)
      likely = "lunch";
  ...
  return Object.entries(buckets)
    .map(([code, timestamps]) => {
      ...
      if (sorted.length >= 2) {
        ...
        if (gaps.length)
          avgI = Math.round(gaps.reduce((a, b) => a + b) / gaps.length);
      }
      // durationSeconds = SAM × 60, totalSam = SAM in minutes
      const samSecs =
        master?.durationSeconds ||
        (master?.totalSam ? Math.round(master.totalSam * 60) : null);

      // avgI is null when there aren't ≥2 real (time-separated) scans — e.g.
      // bulk "mark as done" writes stamp identical timestamps, so every gap is
      // 0 and gets filtered out. In that case we CANNOT measure real cycle
      // time. Per your request we return 0 (not null) so the UI shows a number,
      // but we flag noTimingData so it can be labeled "no scan data" rather than
      // misread as a genuine 0% performance score.
      const hasTiming = !!(samSecs && avgI && avgI > 0);
      const effPct = hasTiming ? Math.round((samSecs / avgI) * 100) : 0;
      return {
        opCode: code,
...
function calcPerformanceScore({
  ...
  const effs = (operationsBreakdown || [])
    .filter((o) => o.efficiencyPct !== null)
    .map((o) => o.efficiencyPct);
  score += effs.length
    ? Math.min(30, (effs.reduce((a, b) => a + b) / effs.length / 100) * 30)
    : 15;
  if (avgIntervalSecs)
    score += Math.max(0, 1 - (avgIntervalSecs - 120) / 480) * 20;
  else score += 10;
...
async function loadOpMaster() {
  ...
  try {
    ...
    const ops = await Operation.find({})
      .select("operationCode name totalSam durationSeconds machineType")
      .lean();
    ops.forEach((op) => {
...
router.get("/overview", ceoAuth, async (req, res) => {
  try {
    ...
    const doc = await ProductionTracking.findOne({
      date: utcRange(getTodayIST()),
    }).lean();
    let activeNow = 0,
      signedOut = 0,
      totalScans = 0;
    ...
    if (doc) {
      for (const m of doc.machines || []) {
        ...
        for (const op of m.operators || []) {
          if (!op.signOutTime) activeNow++;
          else signedOut++;
          totalScans += op.barcodeScans?.length || 0;
          if ((op.barcodeScans?.length || 0) > 0) had = true;
        }
        if (had || m.currentOperatorIdentityId)
          actMach.add(m.machineId?.toString());
      }
    }
...
router.get("/live", ceoAuth, async (req, res) => {
  try {
    ...
    const wos = await WorkOrder.find({
      ...
      .select("workOrderNumber stockItemName _id")
      .lean();
    const woMap = {};
    wos.forEach((w) => {
      woMap[w._id.toString().slice(-8)] = w;
    });

    const machMap = {};
    allMachines.forEach((m) => {
      machMap[m._id.toString()] = m;
    });
    ...
    for (const e of attendanceDoc?.employees || []) {
      ...
      if (e.identityId && e.identityId !== e.biometricId)
        attMap.set(e.identityId, entry);
    }

    // Employee lookup map (batch — avoids N+1)
    const allEmps = await Employee.find({ isActive: true })
      .select(
        "firstName lastName identityId biometricId profilePhoto needsToOperate",
      )
      .lean();
    ...
    for (const machine of doc.machines || []) {
      ...
      for (const op of machine.operators || []) {
        ...
        const name =
          op.operatorName?.trim() && op.operatorName !== "Unknown Operator"
            ? op.operatorName.trim()
            : empData?.name?.trim() || opId || "Unknown";

        const scans = op.barcodeScans || [];
        const signInDateStr = op.signInTime
          ? new Date(op.signInTime).toISOString().split("T")[0]
          : null;
        ...
        const lastOps = Array.isArray(sorted[0]?.activeOps)
          ? sorted[0].activeOps
          : typeof sorted[0]?.activeOps === "string"
            ? sorted[0].activeOps.split(",").filter(Boolean)
            : [];
        ...
        scans.forEach((s) => {
          ...
          if (!woBuckets[parsed.shortId])
            woBuckets[parsed.shortId] = {
              shortId: parsed.shortId,
              woNumber: wo?.workOrderNumber,
              woName: wo?.stockItemName,
              units: new Set(),
              scans: 0,
            };
      ...
      if (machScans > 0 || machine.currentOperatorIdentityId)
        globalActiveMach++;
      ...
      if (machine.currentOperatorIdentityId) {
        ...
        curr = {
          identityId: cId,
          name: cSess?.name || cEmp?.name || cId,
          profilePhoto: cSess?.profilePhoto || cEmp?.profilePhoto || null,
        };
      }
      ...
      for (const op of opsData) {
        ...
        if (
          op.signInTime &&
          (!s.earliestSignIn ||
            new Date(op.signInTime) < new Date(s.earliestSignIn))
        )
          s.earliestSignIn = op.signInTime;
        if (
          op.effectiveSignOut &&
          (!s.latestSignOut ||
            new Date(op.effectiveSignOut) > new Date(s.latestSignOut))
        )
          s.latestSignOut = op.effectiveSignOut;
      ...
      machinesData.push({
        ...
        status: machine.currentOperatorIdentityId
          ? "busy"
          : machScans > 0
            ? "used_today"
            : "free",
        currentOperator: curr,
        operators: opsData.sort(
          (a, b) => (a.isActive ? -1 : 1) - (b.isActive ? -1 : 1),
        ),
    ...
    res.json({
      ...
      summary: {
        totalOperators: machinesData.reduce(
          (s, m) => s + m.operators.length,
          0,
        ),
  ...
  try {
    ...
    const machines = allM.map((m) => {
      const id = m._id.toString(),
        busy = activeMap.get(id) ?? false;
      let status = "free";
      if (m.status === "Under Maintenance" || m.status === "maintenance")
        status = "maintenance";
      else if (m.status === "Offline" || m.status === "offline")
        status = "offline";
      ...
      return {
        ...
        location: m.location,
        status,
        isActiveToday: activeMap.has(id),
      };
    });
...
router.get("/operators", ceoAuth, async (req, res) => {
  try {
    ...
    const allOps = await Employee.find({ needsToOperate: true, isActive: true })
      .select(
        "firstName lastName identityId biometricId department designation profilePhoto",
      )
      .lean();

    const [doc, allMachines] = await Promise.all([
      ProductionTracking.findOne({ date: utcRange(dateStr) }).lean(),
      require("../../models/CMS_Models/Inventory/Configurations/Machine")
        .find({})
        .select("name _id")
        .lean(),
    ]);

    const machNameMap = new Map(
      allMachines.map((m) => [m._id.toString(), m.name]),
    );
      ...
      for (const m of doc.machines || []) {
        for (const op of m.operators || []) {
          ...
          if (op.barcodeScans?.length) {
            ...
            if (iv) a.avgI = iv;
            const sorted = [...op.barcodeScans].sort(
              (a, b) => new Date(b.timeStamp) - new Date(a.timeStamp),
            );
            if (
              sorted[0] &&
              (!a.lastScan ||
                new Date(sorted[0].timeStamp) > new Date(a.lastScan))
            )
    ...
    const result = allOps
      .map((e) => {
        const name = `${e.firstName || ""} ${e.lastName || ""}`.trim();
        const activity =
          aMap.get(e.identityId) || aMap.get(e.biometricId) || null;
        ...
        return {
          ...
          hasActivityToday:
            (activity?.totalScans || 0) > 0 || activity?.isActiveNow || false,
          ...
          machinesUsed: mIds,
          machineName: mIds.length
            ? machNameMap.get(mIds[mIds.length - 1]) || null
            : null,
        };
      })
      .sort((a, b) => {
        if (a.isActiveNow !== b.isActiveNow) return a.isActiveNow ? -1 : 1;
        if (a.hasActivityToday !== b.hasActivityToday)
          return a.hasActivityToday ? -1 : 1;
...
router.get("/operator-detail/:operatorId", ceoAuth, async (req, res) => {
  try {
    ...
    const attEntry =
      (attendanceDoc?.employees || []).find(
        (e) => e.biometricId === operatorId || e.identityId === operatorId,
      ) || null;
    ...
    const sortedScans = [...allScansFlat].sort(
      (a, b) => new Date(a.timeStamp) - new Date(b.timeStamp),
    );
    ...
    const morningScans = allScansFlat.filter(
    ...
    const workOrders = Object.values(woAgg)
      .map((w) => ({ shortId: w.shortId, units: w.units.size, scans: w.scans }))
    ...
    if (prevTrackingDoc) {
      ...
      for (const m of prevTrackingDoc.machines || [])
        for (const op of m.operators || []) {
          ...
          const ms = op.signOutTime
            ? new Date(op.signOutTime) - new Date(op.signInTime)
            : 0;
          prevMs += Math.max(0, ms);
        }
      const pPieces = countUniquePieces(prevScans);
      const pSorted = [...prevScans].sort(
        (a, b) => new Date(a.timeStamp) - new Date(b.timeStamp),
      );
      let pAvgI = null;
      if (pSorted.length >= 2) {
        ...
        for (let i = 1; i < pSorted.length; i++) {
          const d =
            (new Date(pSorted[i].timeStamp) -
              new Date(pSorted[i - 1].timeStamp)) /
            1000;
      ...
      const pScore = calcPerformanceScore({
        uniquePieces: pPieces,
        sessionMins: prevMs / 60000,
        avgScanIntervalSecs: pAvgI,
        operationsBreakdown: pOpsBreak,
      });

      previousDate = {
        date: prevDateStr,
        totalScans: prevScans.length,
        uniquePieces: pPieces,
        avgScanIntervalSecs: pAvgI,
        performanceScore: pScore,
        sessionMins: Math.round(prevMs / 60000),
      };
    }
    ...
    try {
      ...
      const allWOs2 = await WorkOrder.find({})
        .select(
          "_id workOrderNumber stockItemId stockItemName variantAttributes quantity",
        )
        .lean();
      const woByShortId2 = new Map(
        allWOs2.map((wo) => [wo._id.toString().slice(-8), wo]),
      );
      const siIds2 = new Set(
        workOrders
          .map((w) => woByShortId2.get(w.shortId)?.stockItemId?.toString())
          .filter(Boolean),
      );
      const stockItems2 =
        siIds2.size > 0
      ...
      productBreakdown = workOrders
        .map((w) => {
          const wo = woByShortId2.get(w.shortId);
          const si = wo?.stockItemId
            ? siMap2.get(wo.stockItemId.toString())
            : null;
          let image = null;
          ...
          const siOps = (si?.operations || []).filter(
            (op) => !!op.operationCode,
          );
          const opsForProduct = siOps.map((siOp) => {
            const ob = operationsBreakdown.find(
              (o) => o.opCode === siOp.operationCode,
            );
            const samSecs = siOp.totalSeconds || 0;
            const avgActual = ob?.avgIntervalSecs || null;
            const effPct =
              samSecs && avgActual
                ? Math.round((samSecs / avgActual) * 100)
                : null;
            return {
    ...
    if (productBreakdown.length > 0) {
      ...
      for (const ob of operationsBreakdown) {
        if ((!ob.samSecs || ob.samSecs === 0) && siSamMap.has(ob.opCode)) {
          ...
          if (ob.avgIntervalSecs)
            ob.efficiencyPct = Math.round(
              (ob.samSecs / ob.avgIntervalSecs) * 100,
            );
        }
      }
    }
    ...
    res.json({
      ...
      aggregated: {
        ...
        sessionMins: Math.round(sessionMins),
        avgScanIntervalSecs,
        firstScan,
        lastScan,
        morningScans,
        ...
        totalIdleMins: idlePeriods.reduce((s, i) => s + i.durationMins, 0),
      },
      performanceScore,
      hourlyProductivity,
      operationsBreakdown,
...
router.get("/compare-operators", ceoAuth, async (req, res) => {
  try {
    ...
    if (!op1 || !op2)
      return res
        .status(400)
        .json({ success: false, message: "op1 and op2 required" });

    const { ProductionTracking, Employee } = getModels();
    const DailyAttendance = require("../../models/HR_Models/Dailyattendance");

    const [opMasterMap, trackingDoc, attendanceDoc, allEmps] =
      await Promise.all([
        loadOpMaster(),
        ProductionTracking.findOne({ date: utcRange(dateStr) }).lean(),
        DailyAttendance.findOne({ dateStr }).lean(),
        Employee.find({ isActive: true })
          .select(
            "firstName lastName identityId biometricId profilePhoto department",
          )
          .lean(),
      ]);
    ...
    const buildStats = async (opId) => {
      ...
      for (const m of trackingDoc?.machines || [])
        for (const op of m.operators || []) {
          ...
          if (!eff) {
            ...
            if (att?.finalOut) eff = att.finalOut;
          }
          const ms = eff
            ? new Date(eff) - new Date(op.signInTime)
            : !op.signOutTime
              ? Date.now() - new Date(op.signInTime).getTime()
              : 0;
      ...
      const score = calcPerformanceScore({
        uniquePieces: pieces,
        sessionMins: totalMs / 60000,
        avgScanIntervalSecs: avgI,
        operationsBreakdown: opsBreak,
      });
      return {
        identityId: opId,
        name: empData.name,
        ...
        uniquePieces: pieces,
        sessionMins: Math.round(totalMs / 60000),
        avgScanIntervalSecs: avgI,
        performanceScore: score,
        operationsBreakdown: opsBreak,
        hourlyProductivity: buildHourlyProductivity(allScans),
      };
    };
...
router.get("/top-performers", ceoAuth, async (req, res) => {
  try {
    ...
    const [opMasterMap, trackingDoc, attendanceDoc, allEmps, allMachines] =
      await Promise.all([
        loadOpMaster(),
        ProductionTracking.findOne({ date: utcRange(dateStr) }).lean(),
        DailyAttendance.findOne({ dateStr }).lean(),
    ...
    for (const machine of trackingDoc?.machines || []) {
      ...
      for (const op of machine.operators || []) {
        ...
        (op.barcodeScans || []).forEach((s) => {
          ...
          const p = s.barcodeId?.split("-");
          if (p && p[0] === "WO" && p.length >= 3)
            agg.pieces.add(`${p[1]}-${p[2]}`);
        });
        agg.machines.add(mName);
        if (
          !agg.signInTime ||
          new Date(op.signInTime) < new Date(agg.signInTime)
        )
          agg.signInTime = op.signInTime;
        let eff = op.signOutTime;
        if (!eff) {
    ...
    for (const [id, agg] of opAgg) {
      ...
      const score = calcPerformanceScore({
        uniquePieces: agg.pieces.size,
        sessionMins: agg.sessionMs / 60000,
        avgScanIntervalSecs: avgI,
        operationsBreakdown: opsBreak,
      });
      performers.push({
        ...
        profilePhoto: empData.profilePhoto,
        uniquePieces: agg.pieces.size,
        totalScans: agg.scans.length,
        sessionMins: Math.round(agg.sessionMs / 60000),
        avgScanIntervalSecs: avgI,
        performanceScore: score,
        machines: Array.from(agg.machines),
        signInTime: agg.signInTime,
        topOperation: opsBreak[0] || null,
        operationsBreakdown: opsBreak,
      });
    }
...
router.get("/work-orders", ceoAuth, async (req, res) => {
  try {
    ...
    const wos = await WorkOrder.find(q)
      .select(
        "workOrderNumber stockItemName status quantity customerName variantAttributes operations productionCompletion timeline createdAt",
      )
      .sort({ updatedAt: -1 })
...
router.get("/products", ceoAuth, async (req, res) => {
  try {
      ...
      for (const machine of doc.machines || [])
        for (const op of machine.operators || [])
          for (const s of op.barcodeScans || []) {
            ...
            const sid = p[1],
              unit = p[2];
            if (!floorScanMap.has(sid))
              floorScanMap.set(sid, { units: new Set(), totalScans: 0 });
            floorScanMap.get(sid).units.add(unit);
            floorScanMap.get(sid).totalScans++;
            totalFloorScans++;
          }

    if (!floorScanMap.size)
      return res.json({
        success: true,
        totalFloorScans: 0,
    ...
    const allWOs = await WorkOrder.find({})
      .select(
        "_id workOrderNumber customerRequestId stockItemId stockItemName variantAttributes quantity status productionCompletion",
      )
      .lean();
    ...
    for (const [sid, scanData] of floorScanMap) {
      ...
      if (!moAgg.has(moId))
        moAgg.set(moId, { moId, products: new Map(), totalUnitsScanned: 0 });
      ...
      if (!entry.products.has(pk))
        entry.products.set(pk, {
          ...
          productName: wo.stockItemName || "Unknown",
          variantAttributes: wo.variantAttributes || [],
    ...
    const manufacturingOrders = Array.from(moAgg.entries())
      .map(([moId, entry]) => {
    ...
    res.json({
      success: true,
      totalFloorScans,
      totalUnitsInProgress: manufacturingOrders.reduce(
        (s, m) => s + m.totalUnitsScanned,
        0,
      ),
      manufacturingOrders,
      dateRange: { start: startUTC, end: endUTC },
...
router.get("/machine-detail/:machineId", ceoAuth, async (req, res) => {
  try {
    ...
    const machInfo = await Machine.findById(machineId)
      .select("name serialNumber type location status")
      .lean()
      .catch(() => null);
    ...
    const [trackingDoc, attendanceDoc, allEmps] = await Promise.all([
      ...
      DailyAttendance.findOne({ dateStr }).lean(),
      Employee.find({ isActive: true })
        .select("firstName lastName identityId biometricId profilePhoto")
        .lean(),
    ]);
    ...
    if (!trackingDoc)
      return res.json({
        ...
        attendanceNote: !attendanceDoc ? "No attendance data yet" : null,
      });

    const machTracking = (trackingDoc.machines || []).find(
    ...
    for (const op of machTracking.operators || [])
      for (const s of op.barcodeScans || []) {
        const p = s.barcodeId?.split("-");
        if (p && p[0] === "WO" && p.length >= 3) allWoIds.add(p[1]);
      }
    ...
    const getAttStatus = (opId) => {
      ...
      const now = new Date(),
        lO = att.lunchOut ? new Date(att.lunchOut) : null,
        lI = att.lunchIn ? new Date(att.lunchIn) : null,
        tO = att.teaOut ? new Date(att.teaOut) : null,
        tI = att.teaIn ? new Date(att.teaIn) : null,
        fO = att.finalOut ? new Date(att.finalOut) : null;
      if (fO && fO < now)
        return {
          status: "checked_out",
          breakInfo: {
      ...
      if (lO && (!lI || lI < lO))
        return {
          status: "lunch_break",
          breakInfo: {
            label: "On lunch since",
            time: att.lunchOut,
            icon: "lunch",
            mightBeStale: now - lO > 45 * 60000,
          },
        };
      if (tO && (!tI || tI < tO))
        return {
          status: "tea_break",
          breakInfo: {
            label: "On tea break since",
            time: att.teaOut,
            icon: "tea",
            mightBeStale: now - tO > 30 * 60000,
          },
        };
    ...
    const operators = (machTracking.operators || []).map((op) => {
      ...
      for (const [sid, units] of Object.entries(woBuckets)) {
        ...
        const si = wo.stockItemId
          ? siMap2.get(wo.stockItemId.toString())
          : null;
        const mo = wo.customerRequestId
          ? moMap2.get(wo.customerRequestId.toString())
          : null;
        const pk = `${wo.stockItemId || sid}_${(wo.variantAttributes || []).map((v) => `${v.name}:${v.value}`).join("|")}`;
        if (!productMap.has(pk))
          productMap.set(pk, {
            productKey: pk,
            name: si?.name || wo.stockItemName || "Unknown",
            image: resolveImage(si, wo.variantAttributes),
            genderCategory: si?.genderCategory || null,
            variantAttributes: wo.variantAttributes || [],
      ...
      return {
        ...
        signOutTime: op.signOutTime || null,
        effectiveSignOut,
        didNotSignOutFromMachine: didNotSignOut,
        attendanceResolvedSignOut: attResolved,
        isSignedIn,
        isStale,
        ...
        breakInfo,
        attendance: attMap.get(opId) || null,
      };
    });
    ...
    try {
      ...
      const allMachScans = (machTracking.operators || []).flatMap(
        (op) => op.barcodeScans || [],
      );
      ...
      for (const op of machTracking.operators || []) {
        ...
        perOperatorOpsBreakdown[opId] = buildOperationsBreakdown(
          op.barcodeScans,
          opMM,
        );
      }
    ...
    res.json({
      ...
      attendanceLastSynced: attendanceDoc?.syncedAt || null,
      attendanceNote: !attendanceDoc
        ? "No attendance data yet — HR sync may be pending"
        : null,
    });
  } catch (err) {
...
router.get("/floor-data", ceoAuth, async (req, res) => {
  try {
    ...
    try {
      ...
      const layout = await CanvasLayout.findOne({
    ...
    for (const machine of doc?.machines || []) {
      ...
      for (const op of machine.operators || []) {
        ...
        if (
          effectiveSignOut &&
          (!s.latestSignOut ||
            new Date(effectiveSignOut) > new Date(s.latestSignOut))
        )
          s.latestSignOut = effectiveSignOut;
      ...
      for (const op of machine.operators || [])
        for (const s of op.barcodeScans || []) {
          const p = s.barcodeId?.split("-");
          if (p && p[0] === "WO" && p.length >= 3)
            uniqueMachPieces.add(`${p[1]}-${p[2]}`);
        }

      const currOpId = machine.currentOperatorIdentityId;
      const currEmp = currOpId ? empMap.get(currOpId) : null;
      const currActive = Array.from(opSummaryMap.values()).find(
        (o) => o.identityId === currOpId && o.isActiveNow,
      );
...
router.get("/daily-summary", ceoAuth, async (req, res) => {
  try {
    ...
    startUTC.setUTCDate(startUTC.getUTCDate() - LOOKBACK_DAYS);

    // ── 1.  Parallel load ─────────────────────────────────────────────────
    const [trackingDocs, attendanceDoc, allEmps, allMachinesRaw, allWOs] =
      await Promise.all([
    ...
    const todayDoc =
      trackingDocs.find((d) => {
        ...
        return (
          ds === dateStr || new Date(d.date).getTime() === todayUTC.getTime()
        );
      }) || null;
    ...
    for (const e of attendanceDoc?.employees || []) {
      ...
      if (e.lunchOut && e.lunchIn && new Date(e.lunchIn) > new Date(e.lunchOut))
        windows.push({
          start: new Date(e.lunchOut),
          end: new Date(e.lunchIn),
          type: "Lunch",
        });
      if (e.teaOut && e.teaIn && new Date(e.teaIn) > new Date(e.teaOut))
        windows.push({
          start: new Date(e.teaOut),
          end: new Date(e.teaIn),
          type: "Tea",
        });
      const att = {
        finalOut: e.finalOut || null,
        inTime: e.inTime || null,
        lunchOut: e.lunchOut || null,
        lunchIn: e.lunchIn || null,
        teaOut: e.teaOut || null,
        teaIn: e.teaIn || null,
      };
      const put = (key) => {
        breakWindowMap.set(key, windows);
        attDataMap.set(key, att);
      };
    ...
    const siIds = new Set(
      allWOs.map((w) => w.stockItemId?.toString()).filter(Boolean),
    );
    ...
    const validateScan = (barcodeId, activeOps) => {
      ...
      if (!p || p[0] !== "WO" || p.length < 3)
        return { valid: false, reason: "Bad format — expected WO-{id}-{unit}" };
      const shortId = p[1];
      const unitNum = parseInt(p[2], 10);
      if (isNaN(unitNum))
        return { valid: false, reason: "Non-numeric unit number" };
      const wo = woByShortId.get(shortId);
      if (!wo)
        return {
          valid: false,
          reason: `Work order not found (ID: ${shortId})`,
        };
      if (unitNum < 1 || unitNum > (wo.quantity || 0))
        return {
          valid: false,
          reason: `Unit #${unitNum} exceeds WO quantity (${wo.quantity || "?"})`,
        };
      const siId = wo.stockItemId?.toString();
      const knownOps = siId ? siOpsSet.get(siId) || new Set() : new Set();
      const unknownOps = (activeOps || []).filter(
        (c) => knownOps.size > 0 && !knownOps.has(c),
      );
      if (unknownOps.length > 0)
        return {
          valid: true,
          warning: `Op codes not in product definition: ${unknownOps.join(", ")}`,
        };
      if (!activeOps || activeOps.length === 0)
        return {
          valid: true,
          warning: "No active operation on machine at scan time",
        };
      return { valid: true };
    };
      ...
      for (const machine of doc.machines || []) {
        ...
        for (const op of machine.operators || []) {
          ...
          const opName =
            op.operatorName?.trim() && op.operatorName !== "Unknown Operator"
              ? op.operatorName.trim()
              : empInfo?.name || opId || "Unknown";

          for (const scan of op.barcodeScans || []) {
            const p = scan.barcodeId?.split("-");
            if (!p || p[0] !== "WO" || p.length < 3) continue;
            const shortId = p[1],
              unitNum = parseInt(p[2], 10);
            ...
            const scanEntry = {
              ...
              barcodeId: scan.barcodeId,
              isValid: vr.valid,
              warning: vr.warning || null,
              invalidReason: vr.valid ? null : vr.reason,
            };
    ...
    const hourlyMap = {};
    for (let h = 0; h <= 23; h++)
      hourlyMap[h] = { hour: h, pieces: 0, scans: 0, operatorSet: new Set() };

    // First scan per piece (today)
    const firstScanTsToday = new Map();
    for (const [key, ph] of pieceHist) {
      if (!ph.todayScans.length) continue;
      const earliest = ph.todayScans.reduce(
        (mn, s) =>
          !mn || new Date(s.timeStamp) < new Date(mn) ? s.timeStamp : mn,
        null,
      );
      ...
      for (const machine of doc.machines || []) {
        for (const op of machine.operators || []) {
          ...
          for (const scan of op.barcodeScans || []) {
            ...
            if (hourlyMap[h]) {
              hourlyMap[h].scans++;
              hourlyMap[h].operatorSet.add(op.operatorIdentityId);
            }
          }
        }
      }
    ...
    for (const [code, byOp] of opTimingsToday) {
      ...
      for (const [opId, ts] of byOp) {
        ...
        breakdown.push({
          ...
          piecesHandled: ts.length,
          avgSecs: opAvg,
          sampleCount: gaps.length,
        });
      }
      opStats.set(code, {
        avgSecs: safeAvg(allGaps),
        sampleCount: allGaps.length,
        operatorBreakdown: breakdown.sort(
          (a, b) => b.piecesHandled - a.piecesHandled,
        ),
      });
    }
    ...
    for (const [shortId, { wo, pieces }] of woGroups) {
      ...
      const operationsData = expectedOps.map(
        ({ opCode, opName, samSeconds, samMins, machineType }) => {
          ...
          return {
            ...
            piecesWithOp,
            todayScansForOp:
              stats?.operatorBreakdown?.reduce(
                (s, b) => s + b.piecesHandled,
                0,
              ) || 0,
      ...
      const extraOps = [...allScannedCodes]
        ...
        .map((code) => {
          ...
          return {
            ...
            operatorBreakdown: st?.operatorBreakdown || [],
          };
        });

      // Piece audit
      const pieceAudit = pieces
        .map((ph) => {
          const doneOps = [...ph.opsEverDone].filter((c) => expectedSet.has(c));
          const missingOps = [...expectedSet].filter(
            (c) => !ph.opsEverDone.has(c),
          );
          return {
            unitNum: ph.unitNum,
            barcodeId: ph.barcodeId,
            isValid: ph.validationResult.valid,
            validationNote: ph.validationResult.valid
              ? ph.validationResult.warning || null
              : ph.validationResult.reason,
            ...
            completionPct:
              expectedSet.size > 0
                ? Math.round((doneOps.length / expectedSet.size) * 100)
                : 100,
            todayScansCount: ph.todayScans.length,
            todayScans: ph.todayScans.map((s) => ({
              timeStamp: s.timeStamp,
      ...
      const inProgressPcs = pieceAudit.filter(
        (p) => !p.isComplete && p.operationsEverDone.length > 0,
      ).length;
      const notStartedPcs = (wo.quantity || 0) - pieceAudit.length;
      const todayNewPcs = pieces.filter(
        (ph) => ph.firstSeenDate === dateStr,
      ).length;
      const dateRange = pieces.reduce(
        (r, ph) => ({
          from: ph.firstSeenDate < r.from ? ph.firstSeenDate : r.from,
          to: ph.lastScannedDate > r.to ? ph.lastScannedDate : r.to,
        }),
    ...
    const productionSummary = Array.from(siGroups.values())
      .map((pg) => ({
        ...pg,
        siOps: undefined,
    ...
    for (const machine of todayDoc?.machines || []) {
      ...
      for (const op of machine.operators || []) {
        ...
        const scanTimes = scans
          ...
          .sort((a, b) => a - b);
        const firstScan = scanTimes[0] || null,
        ...
        const signInSource = op.signInTime
          ? "machine"
          : firstScan
            ? "first_scan"
            : null;
        ...
        let signOut = op.signOutTime || null,
          signOutSource = op.signOutTime ? "machine" : null,
          didNotSignOut = false;
        if (!signOut) {
          if (att?.finalOut) {
            signOut = att.finalOut;
            signOutSource = "attendance";
            didNotSignOut = true;
          } else if (lastScan) {
            signOut = lastScan.toISOString();
            signOutSource = "last_scan";
            didNotSignOut = !isActive;
          }
        }
        ...
        const endMs = isActive
          ? Date.now()
          : signOut
            ? new Date(signOut).getTime()
            : startMs;
        let netMs = Math.max(0, endMs - startMs);
        for (const w of breakWindowMap.get(opId) || []) {
          const oS = Math.max(startMs, w.start.getTime()),
            oE = Math.min(endMs, w.end.getTime());
        ...
        if (!opAgg.has(opId)) {
          ...
          opAgg.set(opId, {
            identityId: opId,
            name: emp?.name || op.operatorName || opId,
            department: emp?.department || null,
            ...
            isActive,
            didNotSignOut,
            netMins: Math.round(netMs / 60000),
            pieces,
            scans: scans.length,
            att: att || null,
          });
          ...
          agg.pieces += pieces;
          agg.scans += scans.length;
          agg.netMins += Math.round(netMs / 60000);
          if (isActive) agg.isActive = true;
          if (
            signIn &&
            (!agg.signIn || new Date(signIn) < new Date(agg.signIn))
          ) {
            agg.signIn = signIn;
            agg.signInSource = signInSource;
          }
          if (
            signOut &&
            (!agg.signOut || new Date(signOut) > new Date(agg.signOut))
          ) {
            agg.signOut = signOut;
            agg.signOutSource = signOutSource;
          }
        }
        const agg = opAgg.get(opId);
        if (!agg.machines.find((m) => m.machineId === mId))
          agg.machines.push({
            machineId: mId,
            machineName: mName,
            pieces,
    ...
    const operatorProductivity = Array.from(opAgg.values())
      .map((op) => ({
        ...op,
        piecesPerHour:
          op.netMins > 0
            ? Math.round((op.pieces / (op.netMins / 60)) * 10) / 10
            : null,
      }))
      .sort((a, b) => b.pieces - a.pieces);
    ...
    for (const machine of todayDoc?.machines || []) {
      ...
      for (const op of machine.operators || []) {
        ...
        const endDt = !op.signOutTime
          ? att?.finalOut
            ? new Date(att.finalOut)
            : lastScan || new Date()
          : new Date(op.signOutTime);
        if (startDt && endDt) {
          const sM = Math.max(
            540,
      ...
      machineUtilization.push({
        machineId: mId,
        machineName: mInfo.name || "Unknown",
        machineType: mInfo.type || null,
        serialNumber: mInfo.serialNumber || null,
        status: machine.currentOperatorIdentityId
          ? "active"
          : usedMins > 0
            ? "used"
            : "free",
        operatorCount: opSet.size,
        utilizationPct: Math.min(
          100,
          Math.round((usedMins / SHIFT_MINS) * 100),
        ),
        activeMins: Math.min(usedMins, SHIFT_MINS),
        pieces: totalPcs,
    ...
    for (const m of allMachinesRaw) {
      const mId = m._id.toString();
      if (trackedSet.has(mId)) continue;
      machineUtilization.push({
        machineId: mId,
        machineName: m.name,
        machineType: m.type,
    ...
    // ── 10.  Alerts ───────────────────────────────────────────────────────
    const alerts = [];
    const noSignOps = operatorProductivity.filter(
      (o) => o.didNotSignOut && !o.isActive,
    );
    if (noSignOps.length)
      alerts.push({
        type: "no_signout",
        severity: "warning",
        msg: `${noSignOps.length} operator${noSignOps.length > 1 ? "s" : ""} did not sign out from machine. Biometric attendance finalOut used as fallback.`,
        names: noSignOps.map((o) => o.name),
      });
    const multiMachOps = operatorProductivity.filter(
      (o) => o.machines.length > 1,
    );
    if (multiMachOps.length)
      alerts.push({
        type: "multi_machine",
        severity: "info",
        msg: `${multiMachOps.length} operator${multiMachOps.length > 1 ? "s" : ""} worked on multiple machines today — check operator tab for transition analysis.`,
        names: multiMachOps.map((o) => `${o.name} (${o.machines.length})`),
      });
    const totalInvalidScans = productionSummary
      .flatMap((pg) => pg.wos)
      .reduce((s, w) => s + w.invalidScansCount, 0);
    if (totalInvalidScans > 0)
      alerts.push({
        type: "invalid_scans",
        severity: "warning",
        msg: `${totalInvalidScans} scan${totalInvalidScans > 1 ? "s" : ""} failed validation today (unit out of range, WO not found, or bad format). Check production tab for details.`,
      });
    const idleMachs = machineUtilization.filter(
      (m) => m.status === "free" && m.utilizationPct === 0,
    );
    if (idleMachs.length > 0)
      alerts.push({
        type: "idle_machines",
        severity: "info",
        msg: `${idleMachs.length} machine${idleMachs.length > 1 ? "s" : ""} had no production activity today.`,
        machines: idleMachs.map((m) => m.machineName),
      });

    // ── 11.  KPIs ─────────────────────────────────────────────────────────
    const totalVerifiedPieces = firstScanTsToday.size;
    const totalScans = (todayDoc?.machines || []).reduce(
      (s, m) =>
        s +
        m.operators.reduce((ss, op) => ss + (op.barcodeScans?.length || 0), 0),
      0,
    );
    res.json({
      success: true,
      ...
      kpis: {
        ...
        activeMachines: machineUtilization.filter((m) => m.isCurrentlyBusy)
          .length,
        usedMachines: machineUtilization.filter((m) => m.activeMins > 0).length,
        totalMachines: allMachinesRaw.length,
        peakHour:
          hourly.length > 0
            ? hourly.reduce((a, b) => (b.pieces > a.pieces ? b : a))
            : null,
      },
      hourly,
      productionSummary,
...
router.get("/product-efficiency", ceoAuth, async (req, res) => {
  try {
    ...
    for (const scan of allScans) {
      if (!pieceOps.has(scan.woShortId))
        pieceOps.set(scan.woShortId, new Map());
    ...
    for (const woShortId of scannedShortIds) {
      ...
      const operationsData = expectedOps.map(
        ({ opCode, opName, samSeconds, samMins, machineType }) => {
          const piecesWithOp = [...pMap.values()].filter((ops) =>
            ops.has(opCode),
          ).length;
          const stats = opStats.get(opCode);
          const avgActual = stats?.avgSecs || null;
      ...
      const allScannedCodes = new Set();
      for (const ops of pMap.values())
        for (const c of ops) allScannedCodes.add(c);
      const extraOps = [...allScannedCodes]
        .filter((c) => !expectedSet.has(c))
        .map((code) => {
          const st = opStats.get(code);
          return {
            ...
            samMins: null,
            machineType: null,
            piecesWithOp: [...pMap.values()].filter((ops) => ops.has(code))
              .length,
            avgActualSecs: st?.avgSecs || null,
            efficiencyPct: null,
            isExtra: true,
            operatorBreakdown: (st?.operatorBreakdown || []).map((br) => ({
              ...br,
              efficiencyPct: null,
            })),
          };
    ...
    const products = Array.from(productGroups.values())
      .map((pg) => ({
        stockItemId: pg.stockItemId,
        productName: pg.productName,
        genderCategory: pg.genderCategory,
    ...
    const uniquePieces = new Set(
      allScans.map((s) => `${s.woShortId}-${s.unitNum}`),
    ).size;
    res.json({
      success: true,
      date: dateStr,
      ...
      summary: {
        ...
        totalPiecesScanned: uniquePieces,
        totalScans: allScans.length,
        uniqueOps: opTimings.size,
      },
    });
  } catch (err) {
...
router.get("/live-products", ceoAuth, async (req, res) => {
  try {
    const { ProductionTracking, WorkOrder } = getModels();

    // Same StockItem path your other routes use (/products, /machine-detail).
    const StockItem = require("../../models/CMS_Models/Inventory/Products/StockItem");

    const dateStr = req.query.date || getTodayIST();

    // utcRange returns { $gte, $lt } — pass it straight into the query.
    const trackingDoc = await ProductionTracking.findOne({
      date: utcRange(dateStr),
    })
      .populate("machines.machineId", "name serialNumber type")
      .lean();

    if (!trackingDoc) {
      return res.json({ success: true, date: dateStr, liveMachines: [] });
    }

    const now = Date.now();
    const windowMs = LIVE_WINDOW_MINUTES * 60 * 1000;

    // "WO-710c4ae3-001" → "710c4ae3"
    const shortIdFromBarcode = (barcodeId) => {
      if (!barcodeId) return null;
      const parts = barcodeId.split("-");
      if (parts[0] !== "WO" || parts.length < 3) return null;
      return parts[1];
    };

    // A machine qualifies if it had ANY barcode scan today (full day, no window,
    // signed-in or not). 0-scan machines (assigned-only, like RUBITA) are
    // excluded — they have no barcode, so no product to resolve. We take each
    // machine's MOST RECENT scan to pick the product/operator/ops to display,
    // and flag whether that scan is within the live window (for the green dot).
    const liveCandidates = [];
    const shortIds = new Set();

    for (const machine of trackingDoc.machines || []) {
      const mId = machine.machineId?._id?.toString();
      if (!mId) continue;

      let best = null; // most recent scan on this machine, across all operators
      for (const op of machine.operators || []) {
        const scans = op.barcodeScans || [];
        if (!scans.length) continue;
        for (const scan of scans) {
          const ts = scan.timeStamp ? new Date(scan.timeStamp).getTime() : 0;
          if (!best || ts > best.ts) {
            ...
            best = {
              ...
              opCodes: codes,
              shortId: shortIdFromBarcode(scan.barcodeId),
            };
          }
        }
      }

      if (!best) continue; // 0 scans today → excluded

      liveCandidates.push({
        machineId: mId,
        machineName: machine.machineId?.name || "Unknown",
        machineType: machine.machineId?.type || "",
        serial: machine.machineId?.serialNumber || "",
        operatorName: best.operatorName,
        opCodes: best.opCodes,
        lastScanAt: new Date(best.ts),
        isLive: now - best.ts <= windowMs, // true = scanned within window
        shortId: best.shortId,
      });
      if (best.shortId) shortIds.add(best.shortId);
    }
    ...
    // ── Resolve WO short id → WorkOrder → StockItem (image) ───────────────────
    ...
    for (const wo of wos) {
      ...
      if (shortIds.has(sid)) woByShortId.set(sid, wo);
    }

    // Batch-load the StockItems for those WOs
    const stockItemIds = [
      ...new Set(
        [...woByShortId.values()]
          .map((wo) => wo.stockItemId?.toString())
          .filter(Boolean),
      ),
    ];
    ...
    const urlOf = (img) =>
      typeof img === "string" ? img : img?.url || img?.src || null;

    // Product images live EITHER on the top-level StockItem.images OR inside a
    // variant's images[]. Some products only have variant images. So: try
    // top-level first; if empty, pass the WO's variantAttributes to match the
    // right variant; if no match, fall back to the first variant that has any
    // image. Returns the first usable URL or null.
    const imageFromStock = (s, variantAttributes = []) => {
      if (!s) return null;

      // 1. Top-level product image
      if (Array.isArray(s.images) && s.images.length) {
        const u = urlOf(s.images[0]);
        if (u) return u;
      }

      if (Array.isArray(s.variants) && s.variants.length) {
        // 2. Variant matching this WO's attributes (color/size/etc.)
        if (variantAttributes.length) {
          const match = s.variants.find((v) =>
            (v.attributes || []).every((a) =>
          ...
          if (match?.images?.length) {
            const u = urlOf(match.images[0]);
            if (u) return u;
          }
        }
        // 3. Any variant that has an image
        const anyV = s.variants.find((v) => v.images?.length > 0);
        if (anyV) {
          const u = urlOf(anyV.images[0]);
          if (u) return u;
        }
      }
      return null;
    ...
    // ── Assemble the live feed ────────────────────────────────────────────────
    const liveMachines = liveCandidates.map((c) => {
      const wo = c.shortId ? woByShortId.get(c.shortId) : null;
      const stock = wo?.stockItemId
        ? stockById.get(wo.stockItemId.toString())
        : null;
      const variantLabel = (wo?.variantAttributes || [])
        .map((v) => v.value)
        .filter(Boolean)
        .join(" / ");
      ...
      return {
        ...
        isLive: c.isLive,
        product: {
          name: wo?.stockItemName || stock?.name || "Unknown product",
          variant: variantLabel || null,
          image: imageFromStock(stock, wo?.variantAttributes || []),
          woNumber:
            wo?.workOrderNumber || (c.shortId ? `WO-${c.shortId}` : null),
        },
      };
    });
    ...
    console.error("[CEO Prod] /live-products:", err);
    return res
      .status(500)
