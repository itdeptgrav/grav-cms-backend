 ...
 *
 * This file also measures three things the older one cannot, which is the
 * reason it exists rather than being folded into it:
 *
 ...
 *   2. A BOUNDED set of route keys. firestoreBandwidth keys on
 *      `req.baseUrl + req.path`, so every distinct :id is its own map entry.
 *      Here the express route pattern is used instead.
 *   3. History. firestoreBandwidth is memory-only, so a deploy erases it. Here
 *      counters are folded into hourly Mongo buckets.
 *
 ...
 * compression ratio and prove the gzip rollout worked.
 *
...
function blank() {
  return {
    calls: 0,
    bytesOut: 0,      // wire bytes we sent (post-gzip)
    bytesRaw: 0,      // pre-gzip JSON size, when known
    bytesIn: 0,       // request body bytes we received
    outBytesDown: 0,  // service-initiated: bytes we pulled in
    outBytesUp: 0,    // service-initiated: bytes we pushed out
    outCalls: 0,
    totalMs: 0,
    errors: 0,
    fsReads: 0,
...
function middleware(req, res, next) {
  ...
  // Pre-gzip size, so the dashboard can show what compression is actually
  // saving instead of us taking it on faith.
  const origJson = res.json;
  res.json = function (body) {
    try { ctx.bytesRaw += Buffer.byteLength(JSON.stringify(body) ?? ""); } catch { /* circular - skip */ }
  ...
  res.on("finish", () => {
    ...
    merge(cell("route", key), ctx);
    merge(cell("consumer", consumerKey(req)), ctx);
...
function merge(dst, src) {
  for (const k of Object.keys(dst)) if (typeof src[k] === "number") dst[k] += src[k];
}
...
// 4 GB is spread across four different clients - axios (Drive proxying,
// biometric devices), googleapis/gaxios (Workspace, Tasks, Gmail), firebase-
// admin (Firestore REST + FCM) and bare fetch. One hook here catches all of
// them; four client-specific hooks would each miss the other three.
//
...
function recordOutbound(host, down, up) {
...
// ---------------------------------------------------------------------------
// Firestore document operations (carried over from firestoreBandwidth.js)
// ---------------------------------------------------------------------------
  ...
  const flush = async () => {
    ...
    for (const [scope, map] of Object.entries(scopes)) {
      for (const [key, c] of map.entries()) {
        const inc = {};
        for (const [f, v] of Object.entries(c)) if (typeof v === "number" && v !== 0) inc[f] = v;
        if (!Object.keys(inc).length) continue;
        ops.push({
          updateOne: {
            filter: { hour, scope, key },
            update: { $inc: inc, $setOnInsert: { hour, scope, key } },
            upsert: true,
          },
        });
        ...
        for (const f of Object.keys(c)) c[f] = 0;
      }
    }
...
module.exports = {
  ...
  asJob,
  fmt,
  _internals: { normalisePath, routeKey, scopes },
};
