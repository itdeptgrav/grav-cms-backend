...
#include <freertos/queue.h> // v5.6.3 — race-free core0 -> core1 result handoff
...
{
  uint32_t gen;
  // v5.6.3 — the fully-composed request URL, built on core 1 before the task
  // exists. The task must NOT call eventsUrl()/heartbeatUrl() itself: those
  // read serverScheme and serverHost, which are Arduino Strings that core 1
  // rewrites during an SRV: re-point. A String assignment frees, mallocs and
  // memcpys, so a task reading one mid-rewrite can dereference freed memory.
  // A private snapshot removes the shared read entirely.
  String url;
  String payload;
};
...
// ─── Update check, off the main loop (v5.6.3) ─────────────────────────────────
// The availability check is a network request to the CMS with a 10s timeout. It
// used to run inline, which meant the scanner stopped reading barcodes for up to
// ten seconds — once an hour on the periodic check, and again on every
// reconnect. A sewing machine operator scanning a piece into a frozen device
// gets no beep and no count, and scans it again.
//
// It is now a background task exactly like the send and the heartbeat: same
// generation guard, same one-request-then-exit shape, same reclaim path. The
// INSTALL is untouched and still blocking — by then the device is deliberately
// out of service and showing a progress bar.
volatile uint32_t updGeneration = 0;
volatile bool updTaskRunning = false;
unsigned long updStartedAt = 0;

// ─── Why a queue and not a volatile flag (v5.6.3) ─────────────────────────────
// The first cut of this handed the result over as plain Strings published by a
// `volatile bool done`. That is NOT a safe cross-core handoff, for three
// separate reasons:
//
//   1. `volatile` orders volatile accesses against one another. It does not stop
//      the compiler sinking the NON-volatile String stores past the flag store,
//      and it emits no memory barrier on Xtensa — so core 1 can observe the flag
//      before the payload.
//   2. An Arduino String is a heap pointer plus a length. Assignment frees,
//      mallocs and memcpys. A reader on the other core can catch the pointer
//      mid-swap and dereference freed memory.
//   3. Worst: reclaimStuckTasks() can bump updGeneration AFTER the task cleared
//      its guard but BEFORE it wrote the Strings, letting a newly spawned task
//      write the very same String objects concurrently. That is heap
//      corruption, which shows up later as an unrelated random crash.
//
// So the result is a privately-owned heap object whose POINTER is passed through
// a FreeRTOS queue. Every String is written by exactly one core while that core
// is the sole owner, xQueueSend/xQueueReceive supply the ordering and the
// barrier, and ownership transfers with the pointer.
struct UpdateJob
{
  uint32_t gen;
  String checkUrl;        // built on core 1 — the task never reads serverHost
  String payload;
  bool wantMachineName;
  String nameUrl;         // built on core 1; empty when a name is already known
};

struct UpdateResult
{
  uint32_t gen;           // re-checked by the receiver, closing the TOCTOU gap
  int8_t verdict;         // 1 = update offered, -1 = none, 0 = no verdict
  String version;
  String url;
  String machineName;     // empty unless this run fetched one
};

QueueHandle_t updResultQueue = NULL;

// ─── Reconnect follow-up (v5.6.3) ─────────────────────────────────────────────
// onWiFiConnected() used to do every piece of post-reconnect work inline, which
// blocked the MAIN LOOP for up to 25s. That starved three things at once:
// nothing could repaint the header, nothing could consume sendBatchNow, and the
// scanner could not process input. Arming a flag is useless if the loop that
// reads the flag cannot run.
//
// So the follow-up work is now a small state machine driven from loop(), ONE
// phase per pass. The loop stays live throughout: the watchdog is fed between
// phases, scans are processed between phases, and the header repaints the
// instant the heartbeat comes back.
enum ReconnectPhase : uint8_t
{
  RC_IDLE = 0,
  RC_WAIT_CLOCK,    // poll for NTP — polled, never slept on
  RC_QUEUE_TIMES,   // reconcile break epoch + rebuild queued timestamps
  RC_UPDATE_CHECK   // spawns the background check; does not block
};
ReconnectPhase reconnectPhase = RC_IDLE;
unsigned long reconnectPhaseAt = 0;
// Same 10s budget the old inline loop had, just not spent blocking.
const unsigned long RECONNECT_CLOCK_WAIT_MS = 10UL * 1000UL;

// Holds the SENDER while queued events still carry unreconstructed times.
//
// This is load-bearing. The old code was correct by accident: the main loop was
// blocked inside onWiFiConnected() for the whole NTP wait, so nothing could
// send. Now that the loop stays live, loop()'s own retry path — which fires on
// QUEUE_RETRY_INTERVAL regardless of sendBatchNow — and kickSender() on a fresh
// scan could both drain the queue BEFORE recoverQueuedTimes() has run, shipping
// events stamped with epoch 0. Draining early is not worth corrupting the times
// on a shift's production data.
bool queueTimesPending = false;
...
// v5.6.3 — set when backendOk actually TRANSITIONS, so the header can repaint
// on the transition instead of waiting for the next 1s tick.
//
// Set from the background tasks on core 0 and cleared by loop() on core 1.
// It is only ever a hint to redraw, never a source of truth, so a lost update
// costs one second of staleness and nothing else. Deliberately NOT a draw call:
// the tasks must never touch the TFT, because the main loop owns that SPI bus
// and two writers produce a corrupted screen.
volatile bool backendStateDirty = false;

bool isWiFiConnected(); // defined below; needed by the guard in noteBackendResult
...
void noteBackendResult(bool ok)
{
  if (ok)
  {
    // v5.6.3 — a success that LANDS AFTER THE LINK DROPPED describes a network
    // that no longer exists.
    //
    // The window is real: the task reads the body, closes the socket and frees
    // its job between getting the 200 and arriving here, and the link can go
    // during that. checkWiFiConnection() would already have forced the cross,
    // and it only acts on the TRANSITION — so this late success would flip the
    // tick back to green and nothing would ever correct it, because heartbeats
    // are gated on the link being up. The result is a green "server reachable"
    // tick sitting next to a WiFi icon with no bars, for the whole outage.
    //
    // No association means unreachable, full stop. This is the one case that
    // does not need the anti-flap threshold: that exists to absorb dropped
    // packets, not a missing link.
    if (!isWiFiConnected())
      return;

    bool wasDown = !backendOk;
    backendFailStreak = 0;
    backendOk = true;
    // RECOVER FAST: one good response is proof. No second heartbeat required.
    if (wasDown)
    {
  ...
  // FAIL SLOW: unchanged threshold, so one dropped packet still cannot flap the
  // indicator. Only the logging is new, and only on the transition.
  if (backendFailStreak >= BACKEND_FAIL_THRESHOLD && backendOk)
  {
    backendOk = false;
...
void updateCheckTaskFn(void *param)
{
  ...
  {
    HTTPClient http;
    http.begin(job->checkUrl);
    http.addHeader("Content-Type", "application/json");
    // REQUIRED in a task. vTaskDelete(NULL) never returns, so ~HTTPClient()
    // never runs; with reuse on, only the destructor would close the socket and
    // lwIP's 16 sockets leak away. Same fix as the send and heartbeat tasks.
    http.setReuse(false);
    ...
    if (code == 200)
    {
      ...
      if (!deserializeJson(r, resp))
      {
        ...
        if (upd && r.containsKey("firmware"))
        {
          ...
          String ver = fw["version"].as<String>();
          String url = fw["url"].as<String>();
          if (ver.length() > 0 && url.length() > 0)
          {
            // Skip a version that already failed on this boot. Without this a
            // bad upload — wrong file, truncated transfer — is retried on every
            // check and after every watchdog reboot, which is what made one bad
            // file look like an endless download loop.
            if (otaFailedVersion.length() > 0 && ver == otaFailedVersion)
            {
              Serial.println("[OTA] Skipping " + ver + " — already failed this boot");
              res->verdict = -1;
            }
            else
            {
  ...
  // ── 2. machine name, only when core 1 said we still need one ──────────────
  // Moved off the main loop: this was a blocking 5s GET that froze barcode
  // input after every reconnect on a device whose name had never been fetched.
  // The NVS write stays on core 1 — Preferences is not something to poke from
  // a background task while the main loop may also be persisting.
  if (job->wantMachineName && job->nameUrl.length() > 0)
  {
    HTTPClient http;
    http.begin(job->nameUrl);
    http.setReuse(false);
    http.setTimeout(5000);
    int code = http.GET();
  ...
  // Publish by ownership transfer. If the queue is gone or full, nobody will
  // ever collect this, so free it here rather than leak.
  if (updResultQueue == NULL || xQueueSend(updResultQueue, &res, 0) != pdTRUE)
  {
    Serial.println("[OTA] Update-check result dropped (no consumer)");
    delete res;
  }
...
// Spawns the check and returns. Never blocks the caller.
bool startUpdateCheck()
{
  if (!isWiFiConnected() || deviceId.length() == 0 || updTaskRunning)
    return false;

  if (updResultQueue == NULL)
  {
    updResultQueue = xQueueCreate(1, sizeof(UpdateResult *));
    if (updResultQueue == NULL)
    {
      Serial.println("[OTA] Result queue alloc failed");
      return false;
    }
  }

  // Clear any result left in the one-deep mailbox by a task the reclaim
  // watchdog disowned. Draining HERE — on core 1, before the next task exists —
  // guarantees the slot is free, so a stale entry can no longer cause the run
  // we are about to start to be dropped. Safe by construction: only core 1
  // receives, and no producer exists at this instant.
  UpdateResult *stale = NULL;
  while (xQueueReceive(updResultQueue, &stale, 0) == pdTRUE)
  {
    if (stale)
    {
      Serial.println("[OTA] Dropping stale update-check result");
      delete stale;
    }
    stale = NULL;
  }
  ...
  // Every URL and String is composed HERE, on core 1, before the task exists.
  // The task therefore never reads serverScheme/serverHost/machineId, so an
  // SRV: re-point cannot mutate a String out from under it.
  job->checkUrl = deviceRegisterUrl();
  job->wantMachineName = (machineName.length() == 0 && machineId.length() > 0);
  job->nameUrl = job->wantMachineName ? (machineNameUrl() + machineId) : String("");
  ...
  job->gen = ++updGeneration;

  updStartedAt = millis();
  updTaskRunning = true;

  if (xTaskCreatePinnedToCore(updateCheckTaskFn, "updchk", 8192, job, 1, NULL, 0) != pdPASS)
  {
    // Free the payload, release the flag, leave the timestamp clear — the next
    // hourly tick or reconnect retries cleanly.
    delete job;
    updTaskRunning = false;
    updStartedAt = 0;
    Serial.println("[OTA] Update-check task create failed — will retry");
    return false;
  }
  return true;
}

// Main-loop side. updateAvailable / newFirmwareVersion / newFirmwareUrl and the
// NVS machine-name write all stay single-writer on core 1, exactly as before.
void handleUpdateCheckCompletion()
{
  if (updResultQueue == NULL)
    return;
  UpdateResult *res = NULL;
  if (xQueueReceive(updResultQueue, &res, 0) != pdTRUE || res == NULL)
    return;

  // Re-check the generation HERE as well as in the task. The task's own check
  // can be invalidated between clearing it and sending; this one cannot, because
  // only core 1 changes updGeneration and only core 1 runs this.
  if (res->gen != updGeneration)
  {
    Serial.println("[OTA] Stale update-check result discarded");
    delete res;
    return;
  }
  ...
  if (res->verdict == 1)
  {
    newFirmwareVersion = res->version;
    newFirmwareUrl = res->url;
    updateAvailable = true;
    Serial.println("[OTA] Update available: " + newFirmwareVersion);
  }
  else if (res->verdict == -1)
  {
    updateAvailable = false;
  }
  // verdict 0: no answer worth acting on — leave updateAvailable alone, so a
  // timeout cannot clear a pending update.

  if (res->machineName.length() > 0)
    saveMachineName(res->machineName); // NVS write, core 1 only
...
void bulkSendTaskFn(void *param)
{
  ...
  HTTPClient http;
  http.begin(job->url); // private snapshot; never reads serverHost from core 0
  http.addHeader("Content-Type", "application/json");
...
void startBackgroundBulkSend()
{
  if (!isWiFiConnected() || storedScanCount == 0 || isSendingBulk)
    return;
  // Every send funnels through here — loop()'s retry, kickSender() on a scan,
  // and sendBulkScans() — so this one guard covers them all. Bounded by
  // RECONNECT_CLOCK_WAIT_MS, and the events are durable in LittleFS meanwhile.
  if (queueTimesPending)
    return;
  ...
  job->url = eventsUrl(); // snapshot taken here, on core 1
...
void sendHeartbeat()
{
  ...
  job->url = heartbeatUrl(); // snapshot taken here, on core 1
  serializeJson(doc, job->payload);
...
void reclaimStuckTasks()
{
  ...
  if (updTaskRunning && updStartedAt && (now - updStartedAt) > TASK_STUCK_TIMEOUT_MS)
  {
    Serial.println("[Watchdog] Update-check task stuck — releasing flag");
    updGeneration++; // disown whatever it eventually reports
    updTaskRunning = false;
    updStartedAt = 0;
    // Deliberately NOT noteBackendResult(): this request goes to the CMS, not
    // to the scanner ingest server, so it says nothing about backendOk.
  }
...
//
// ─── v5.6.3: this function must RETURN, not work ──────────────────────────────
// It used to run every follow-up task inline:
//
//   fetchMachineNameIfNeeded()   blocking GET,  5s timeout
//   NTP wait loop                50 x delay(200) = 10s, and the FULL 10s on a
//                                LAN with no internet route, forever
//   checkForUpdates()            blocking POST, 10s timeout, plus a 1.2MB OTA
//
// ~25s inside a function called from loop(). Moving sendHeartbeat() to the top
// was not enough on its own: the heartbeat task would succeed on core 0 and set
// backendStateDirty, but the header could not repaint and sendBatchNow could
// not be consumed, because loop() itself was still stuck in here. A flag is
// worthless while its reader is blocked.
//
// Now this function only starts things that do not block, and hands the rest to
// a state machine loop() drives one phase at a time.
void onWiFiConnected()
{
  ...
  // Probe the CONFIGURED production server. Spawns a task and returns.
  Serial.println("[Server] checking configured endpoint immediately");
  sendHeartbeat();

  // Starts SNTP in the background; does not wait for it.
  configTime(GMT_OFFSET_SEC, DAYLIGHT_OFFSET_SEC, "in.pool.ntp.org", "pool.ntp.org", "time.nist.gov");

  // Hold the sender until timestamps are rebuilt, then let loop() take over.
  queueTimesPending = true;
  reconnectPhase = RC_WAIT_CLOCK;
  reconnectPhaseAt = millis();
}

// One phase per loop() pass, so the loop is never held for more than a single
// phase and the header/queue/scanner all stay responsive between them.
void serviceReconnectFollowUp()
{
  if (reconnectPhase == RC_IDLE)
    return;

  // Link went again mid-sequence. Abandon the follow-up and release the send
  // hold — sends are gated on the link anyway, and the next association starts
  // this over. Leaving queueTimesPending set here would wedge the sender.
  if (!isWiFiConnected())
  {
    reconnectPhase = RC_IDLE;
    queueTimesPending = false;
    return;
  }

  // ─── The indicator outranks every phase below ─────────────────────────────
  // No phase blocks any more: the update check and the machine-name fetch both
  // moved into the background task. The gate is kept anyway so that a repaint
  // already owed is never queued behind even task creation — the indicator
  // outranks every piece of secondary work, which is the whole point of this
  // release:
  //
  //   · hbTaskRunning      — the probe is still in flight. Blocking now would
  //                          mean its result lands mid-block and the header
  //                          cannot repaint until the block ends.
  //   · backendStateDirty  — a repaint is already owed. Yield this pass; loop()
  //                          repaints a few statements further down, and the
  //                          phase runs on the next pass with the flag clear.
  //
  // Bounded either way: a heartbeat that never answers is released by
  // reclaimStuckTasks() at 30s, and these phases are secondary by definition.
  if (reconnectPhase == RC_UPDATE_CHECK && (hbTaskRunning || backendStateDirty))
    return;
  ...
  case RC_WAIT_CLOCK:
  {
    // Polled, not slept on. Same 10s ceiling as the old inline loop, but the
    // main loop keeps running through it. On a normal reconnect the clock is
    // already valid and this passes on the very first call.
    time_t n;
    time(&n);
    if (n > 1700000000UL || (millis() - reconnectPhaseAt) >= RECONNECT_CLOCK_WAIT_MS)
      reconnectPhase = RC_QUEUE_TIMES;
    break;
  }

  case RC_QUEUE_TIMES:
    // Fast and local: no network, just LittleFS and RAM.
    reconcileBreakEpochIfNeeded();
    recoverQueuedTimes();
    queueTimesPending = false; // timestamps are now correct — sending is safe
    if (storedScanCount > 0)
    {
      Serial.println("[Queue] draining " + String(storedScanCount) + " queued event(s)");
      sendBatchNow = true;
    }
    reconnectPhase = RC_UPDATE_CHECK;
    break;

  case RC_UPDATE_CHECK:
    startUpdateCheck(); // spawns a task; the result lands via loop()
    reconnectPhase = RC_IDLE;
    break;
...
// v5.6.3 — how long to wait before the next reconnect attempt.
//
// This was a flat 30s. A router that blinked for five seconds therefore cost
// the floor up to another thirty before the device even tried again, showing a
// cross the whole time.
//
// The escalation LADDER below is untouched and still keyed on
// wifiConnectionFailCount — only the spacing between its rungs changes. Early
// attempts are cheap and usually win outright (WiFi.setAutoReconnect(true) is
// frequently succeeding in parallel anyway); a genuinely long outage backs off
// to exactly the old 30s, so a dead AP is still not hammered.
//
//   fails 1-3    3s   short blip, recover in seconds
//   fails 4-8   10s   something real is wrong
//   fails 9+    30s   unchanged: long outage, stop hammering
//
// The idle-restart rung at 40 fails is reached in ~17 min rather than ~20, and
// remains gated on nobody being signed in with an empty queue.
static unsigned long wifiRetryIntervalFor(int fails)
{
  if (fails <= 3) return 3000UL;
  if (fails <= 8) return 10000UL;
  return WIFI_CONNECTION_RETRY_INTERVAL;
}
...
void checkWiFiConnection()
{
  ...
  if (lastWiFiState != cur)
  {
    ...
    else
    {
      ...
      backendOk = false;
      backendFailStreak = BACKEND_FAIL_THRESHOLD;
      // No WiFi means unreachable, so this is the one case that may go red
      // with no failed request behind it — the anti-flap threshold exists to
      // absorb dropped packets, not a missing link.
      Serial.println("[Server] offline - WiFi path unavailable");
      backendStateDirty = true;
    }
    lastWiFiState = cur;
  }

  if (!cur && (now - lastWiFiConnectionAttempt) > wifiRetryIntervalFor(wifiConnectionFailCount))
  {
...
void handleServerUpdateBarcode(const String &s)
{
  ...
  // v5.6.3 — a reconnect follow-up may still be holding the sender while it
  // waits for the clock. Settle it now: the work is local and immediate, and
  // without this the flush below would spin out its full 20s sending nothing.
  if (queueTimesPending)
  {
    reconcileBreakEpochIfNeeded();
    recoverQueuedTimes();
  ...
  // ─── v5.6.3: disown the OLD server's in-flight work ────────────────────────
  // The flush above is bounded (20s) and can return with a send or a heartbeat
  // still running. Those tasks hold a URL built from the OLD scheme/host/port.
  // Left owned, such a task could:
  //
  //   · return 200 from the OLD server and call noteBackendResult(true),
  //     painting the NEW server green before it has ever been contacted; or
  //   · finish a send and let handleBulkSendCompletion() delete queue entries
  //     as "confirmed" — confirmed by a server that no longer owns this device.
  //
  // Bumping both generations makes any such task exit at its own guard without
  // touching shared state. The flags are then released exactly the way
  // handleRefreshSystem() and reclaimStuckTasks() already do it, so the new
  // target can start work at once instead of waiting out the 30s reclaim.
  //
  // Nothing is removed from the queue here: an event the old server may have
  // stored is still unconfirmed as far as THIS device is concerned, so it is
  // re-sent to the new server. eventId keeps that idempotent per server.
  hbGeneration++;
  hbTaskRunning = false;
  hbStartedAt = 0;
  bulkGeneration++;
  ...
  // The new server has not answered yet, so do not inherit the old one's
  // verdict. The heartbeat fired below establishes the truth.
  backendOk = false;
  backendFailStreak = BACKEND_FAIL_THRESHOLD;
  backendStateDirty = true;
...
<input type="number" id="serverPort" placeholder="Port (default %SCANNER_PORT%)">
...
void loop()
{
  ...
  // Post-reconnect follow-up, one phase per pass. Must come AFTER
  // checkWiFiConnection() so a fresh association is serviced on the same pass.
  serviceReconnectFollowUp();
  ...
  // v5.6.3 — a known state change repaints NOW rather than waiting up to a
  // second for the next tick. The flag is cleared only once drawHeader() has
  // actually run, so a repaint blocked by a feedback screen is retried on the
  // next pass instead of being dropped.
  bool statusChanged = backendStateDirty;
  bool headerDue = (now - lastHeaderRefresh) > HEADER_REFRESH_INTERVAL;

  if ((headerDue || statusChanged) && !visualFeedbackActive &&
      !isInBreakMode() && !infoModeActive && !messagePending && scanFeedbackUntil == 0)
  {
    drawQueueLine();
  }

  if ((headerDue || statusChanged) && !visualFeedbackActive && !isInBreakMode())
  {
    drawHeader();
    lastHeaderRefresh = now;
