// models/Accountant_model/Acc_RowPriority.js
//
// Per-user manual sort order for list-page rows.
//
// Distinct from Acc_PinnedItem in intent:
//   - Acc_PinnedItem  → "float this row to the top in an explicit Pinned section"
//   - Acc_RowPriority → "within this bucket, this row should sit at this position"
//
...
//   entityType  — which page (same vocabulary as Acc_PinnedItem)
...
//     1. rank ASC if the row has a Acc_RowPriority entry
...
const rowPrioritySchema = new mongoose.Schema(
  ...
  { timestamps: true, collection: "acc_row_priorities" },
);
...
const Acc_RowPriority =
  mongoose.models.Acc_RowPriority ||
  mongoose.model("Acc_RowPriority", rowPrioritySchema);

module.exports = Acc_RowPriority;
