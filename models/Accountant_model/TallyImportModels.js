// models/Accountant_model/TallyImportModels.js
// =============================================================================
// TALLY IMPORT INFRASTRUCTURE
// -----------------------------------------------------------------------------
// Two models:
//   1. TallyFieldMapping — saved column→field mapping templates (mirrors
//      Tally's own .tsf "mapping templates"). Lets the user re-import the
//      same Excel format next month without re-mapping everything.
//   2. TallyImportSession — one row per import attempt; holds the raw upload,
...
const fieldMappingSchema = new mongoose.Schema(
  {
    ...
    companyId:     { type: mongoose.Schema.Types.ObjectId, ref: "TallyCompany" }, // null = global template
  },
  { timestamps: true, collection: "tally_field_mappings" }
);
...
const tallyImportSessionSchema = new mongoose.Schema(
  {
    ...
    fieldMappingId:   { type: mongoose.Schema.Types.ObjectId, ref: "TallyFieldMapping" },
    ...
    initiatedBy: { type: mongoose.Schema.Types.ObjectId, ref: "AccountantDepartment" },
  },
  { timestamps: true, collection: "tally_import_sessions" }
);
...
// EXPORTS
// ─────────────────────────────────────────────────────────────────────────────
const TallyFieldMapping   = mongoose.model("TallyFieldMapping",   fieldMappingSchema);
const TallyImportSession  = mongoose.model("TallyImportSession",  tallyImportSessionSchema);
