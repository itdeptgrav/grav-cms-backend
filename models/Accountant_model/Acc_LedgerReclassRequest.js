// models/Accountant_model/Acc_LedgerReclassRequest.js
//
...
//                       → server updates Acc_Ledger.groupId / groupOrder
...
const ledgerReclassSchema = new mongoose.Schema(
  {
    ...
    fromGroupId: { type: mongoose.Schema.Types.ObjectId, ref: "Acc_Group" },
  ...
  { timestamps: true, collection: "acc_ledger_reclass_requests" },
);

ledgerReclassSchema.index({ organizationId: 1, status: 1, createdAt: -1 });
ledgerReclassSchema.index({ companyId: 1, ledgerId: 1, status: 1 });

const Acc_LedgerReclassRequest =
  mongoose.models.Acc_LedgerReclassRequest ||
  mongoose.model("Acc_LedgerReclassRequest", ledgerReclassSchema);

module.exports = Acc_LedgerReclassRequest;
