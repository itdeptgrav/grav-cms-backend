// services/production/embroideryStageTarget.service.js
//
// EMBROIDERY'S SIDE OF THE PPC TARGET: SEE IT, ACCEPT IT, OR REFUSE IT.
//
// PPC publishes an embroidery target for one Sales line (services/ppc/
// stagePublication.service.js). This is the only way Embroidery answers it, and
// the only thing Embroidery may write on it: its own acceptance or refusal of
...
// scans one record per finished piece. Those scans are Embroidery's own
// execution actuals and stay exactly where they are: accepting a target
// records no piece, and no number of scans ever amounts to an acceptance.
//
...
// leaves PPC to replan. Embroidery owns this answer: PPC cannot accept on
// Embroidery's behalf, because PPC's own door has no accept command at all.
// Embroidery's piece scans stay exactly where they were, written by the
// existing embroidery scan route; accepting a target records no piece and
// completes nothing.
//
// ── SCOPE ───────────────────────────────────────────────────────────────────
// Every read and write is company-scoped by the acting Embroidery user's own
...
// published a newer one, and that is the version Embroidery is being asked about.
//
// Two more things a target must still be, to be answerable:
//
//   · EMBROIDERY'S. The publication record is the shape every published stage
//     will use, so this door filters on the process rather than trusting that
//     embroidery is the only one — the day sewing is published, a sewing target
//     must not appear in Embroidery's queue or be answerable here.
...
/** The one published process this department answers. A cutting target is
    another department's business: invisible here, and unanswerable. */
...
function refusalProblem(raw) {
  ...
  if (!note) return "Say why Embroidery cannot meet this target.";
...
/** The embroidery targets in force for these WorkOrders, within one company. */
...
/** Embroidery's view of one target: what to cut, by when, and what it answered. */
...
/**
 * Embroidery answers one exact published version.
 *
 * @param decision "ACCEPTED" — Embroidery accepts these dates for this version;
...
async function respond(companyId, publicationId, { decision, reason = "", actor = null } = {}) {
  ...
  if (!isId(publicationId)) throw fail("EMBROIDERY_TARGET_NOT_FOUND", "No embroidery target of yours has that id.");
  const doc = await PpcStagePublication.findOne({
    _id: oid(publicationId), companyId: oid(companyId), process: EMBROIDERY,
  }).lean();
  if (!doc) throw fail("EMBROIDERY_TARGET_NOT_FOUND", "No embroidery target of yours has that id.");

  const note = str(reason).replace(/\s+/g, " ");
  if (wanted === PUBLICATION_STATE.REFUSED) {
    const problem = refusalProblem(note);
    if (problem) throw fail("EMBROIDERY_TARGET_REFUSAL_REASON_REQUIRED", problem, { field: "reason" });
  }
  ...
  if (!live.has(str(doc.planningFileId))) {
    throw fail("EMBROIDERY_TARGET_PLAN_RETIRED",
  ...
  /* ── ONE ANSWER, DECIDED BY THE DATABASE ───────────────────────────────
     Two Embroidery editors answering at once both pass a read-then-check, and
  ...
  const answered = await PpcStagePublication.findOneAndUpdate(
    { _id: doc._id, companyId: oid(companyId), process: EMBROIDERY, isCurrent: true, state: PUBLICATION_STATE.AWAITING },
    {
      $set: {
        state: wanted,
        response: {
  ...
  if (now?.state === PUBLICATION_STATE.SUPERSEDED || !now?.isCurrent) {
    throw fail("EMBROIDERY_TARGET_ALREADY_ANSWERED",
...
module.exports = {
  REFUSAL_MIN, EMBROIDERY, refusalProblem, currentForWorkOrders, targetsByWorkOrder, targetView, respond,
};
