...
/* The account walk no longer takes a model — an unscoped `Model.findById`
   climbs out of the company. It takes the loader the caller has already
   scoped, which here (no route, no actor) is every account. */
const loadAccount = (id) => Account.findById(id).select("parentAccountId").lean();

describe("account hierarchy", () => {
  test("rejects self-parenting", async () => {
    const a = await Account.create({ companyName: "Solo" });
    await expect(assertNoAccountCycle(loadAccount, a._id, a._id)).rejects.toThrow(HierarchyError);
  });
  ...
  test("rejects a direct cycle (A→B then B→A)", async () => {
    ...
    await expect(assertNoAccountCycle(loadAccount, a._id, b._id)).rejects.toThrow(HierarchyError);
  });
  ...
  test("rejects an indirect cycle (A→B→C then A under C)", async () => {
    ...
    await expect(assertNoAccountCycle(loadAccount, a._id, c._id)).rejects.toThrow(HierarchyError);
  });
  ...
  test("allows a legitimate new parent", async () => {
    ...
    await expect(assertNoAccountCycle(loadAccount, child._id, parent._id)).resolves.toBeUndefined();
  });

  test("allows detaching (null parent)", async () => {
    const a = await Account.create({ companyName: "A" });
    await expect(assertNoAccountCycle(loadAccount, a._id, null)).resolves.toBeUndefined();
  });
});
